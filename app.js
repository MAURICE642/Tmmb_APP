...
async function initFB(cfg) {
  ...
  try {
    ...
    // ✅ FIX CONNEXION INSTABLE : cache local persistant (IndexedDB) activé.
    // Sans ceci, chaque coupure/reprise réseau forçait Firestore à re-solliciter
    // le serveur pour tout, provoquant les "saccades" visibles à l'écran.
    // Avec persistentLocalCache, les lectures/écritures utilisent le disque
    // local en priorité et se synchronisent en silence dès que le réseau
    // revient. persistentMultipleTabManager permet d'ouvrir l'app dans
    // plusieurs onglets/PWA sans conflit de verrou IndexedDB.
    try {
      db_fs = initializeFirestore(app, {
        localCache: persistentLocalCache({ tabManager: persistentMultipleTabManager() })
      });
    } catch (persistErr) {
      // Si le cache persistant échoue (navigateur privé, quota, onglet déjà
      // initialisé différemment...), on retombe sur le mode mémoire standard
      // plutôt que de bloquer l'app.
      console.warn('Cache persistant Firestore indisponible, mode mémoire utilisé :', persistErr);
...
// FIX PERF : plafond du nombre de lignes réellement injectées dans le DOM
// pour les tableaux d'historique non paginés (reçus, dépenses, livraisons).
// Sans filtre de date, ces listes grossissent indéfiniment avec l'usage —
// au-delà de ce plafond on affiche un message invitant à filtrer plutôt que
// de forcer le rendu de milliers de lignes d'un coup (source de lenteur/jank
// sur mobile). Les données elles-mêmes ne sont jamais tronquées, seul
// l'affichage l'est.
const _HIST_RENDER_CAP = 300;
...
let _regLastSig = null; // signature (date|agenceId) du dernier contexte réellement affiché — voir renderRegistre()
let _ficheDateQueryToken = 0;
let _ficheLastSig = null; // signature (date|comId) du dernier contexte réellement affiché — voir renderFiche()
...
const _PAGE_DEPS = {
  // ✅ FIX ÉCONOMIE LECTURES : 'paiements' (fenêtre glissante gérée à part,
  // voir _dashPaysFenetre), 'adhesionPays', 'mises', 'pointsJour',
  // 'primesPaliers' et 'versements' retirés — aucune fonction du tableau de
  // bord ne les utilise ; elles ne servent qu'à d'autres pages (registre,
  // fiche, saisie-mises, dépenses commerciaux…) qui les déclarent déjà
  // elles-mêmes et les chargeront à la demande si l'admin s'y rend.
  'admin-dashboard':    ['clients','commerciaux','agences'],
  'tous-clients':       ['paiements','commerciaux'],
  'controle':           ['clients','commerciaux'],
  'articles':           ['articles'],
...
async function _loadOfflineQueue() {
  try {
    // FIX QUOTA : migration transparente localStorage → IndexedDB (les deux
    // clés, chiffrée et non chiffrée, au cas où l'utilisateur avait basculé
    // entre les deux modes dans une version antérieure).
    await _migrateLocalStorageBlobToIdb(_OFFLINE_QUEUE_KEY + '_enc', _OFFLINE_QUEUE_KEY + '_enc');
    await _migrateLocalStorageBlobToIdb(_OFFLINE_QUEUE_KEY, _OFFLINE_QUEUE_KEY);
    const raw = await idbGet(_OFFLINE_QUEUE_KEY + '_enc');
    if (raw && _localKey) {
      _offlineQueue = await _decryptOfflineQueue(raw);
    } else if (raw && !_localKey) {
      // ⚠️ FIX PERTE SILENCIEUSE : une file chiffrée existe mais la clé
      // locale n'est pas encore prête (ex: appelé avant _initLocalKey lors
      // d'une restauration de session). AVANT : on tombait dans le fallback
      // "plain" ci-dessous, qui lisait une clé vide → _offlineQueue = []
      // → les opérations en attente devenaient invisibles, sans erreur ni
      // notification. On NE VIDE PLUS la queue dans ce cas : on la laisse
      // vide EN MÉMOIRE temporairement mais on marque un flag pour
      // réessayer dès que la clé sera disponible, et on prévient (log +
      // badge) au lieu de faire comme si tout allait bien.
      _offlineQueueLoadPending = true;
      console.warn('[offline] File chiffrée détectée mais clé locale indisponible — nouvelle tentative différée.');
      return;
    } else {
      // Fallback non chiffré (mode local / migration)
      const plain = await idbGet(_OFFLINE_QUEUE_KEY);
      _offlineQueue = plain ? JSON.parse(plain) : [];
    }
    _offlineQueueLoadPending = false;
  } catch(e) {
    // ⚠️ FIX PERTE SILENCIEUSE : NE JAMAIS écraser une file existante par
    // un tableau vide sur simple échec de lecture/déchiffrement — ça
    // effaçait potentiellement des écritures utilisateur jamais envoyées
    // au serveur, sans aucune trace. On garde l'état précédent en mémoire
    // (s'il y en avait un dans cette session), on log l'erreur bien
    // visiblement, et on notifie l'utilisateur pour qu'il sache qu'une
    // vérification manuelle peut être nécessaire au lieu de croire, à tort,
    // que tout est synchronisé.
    console.error('[offline] ÉCHEC lecture file offline — file NON réinitialisée pour éviter une perte de données:', e);
    if (typeof notify === 'function') {
      notify('⚠️ Impossible de lire les opérations en attente locale — contactez le support si des saisies récentes semblent manquantes', 'err');
    }
    _offlineQueueLoadPending = true;
  }
  _updateOfflineBadge?.();
}
let _offlineQueueLoadPending = false;
async function _saveOfflineQueue() {
  try {
    if (_localKey) {
      const enc = await _encryptLocal(_offlineQueue);
      if (enc) {
        await idbSet(_OFFLINE_QUEUE_KEY + '_enc', enc);
      } else {
        // ⚠️ FIX PERTE SILENCIEUSE : _encryptLocal a échoué (retourne
        // falsy) sans lever d'exception — avant, l'opération ajoutée à
        // _offlineQueue restait UNIQUEMENT en mémoire : un refresh de page
        // avant le prochain replay la perdait définitivement, sans trace.
        // On retente en clair plutôt que de perdre la sauvegarde.
        console.warn('[offline] Échec chiffrement file offline — repli sur stockage non chiffré pour ne pas perdre l\'opération.');
        await idbSet(_OFFLINE_QUEUE_KEY, JSON.stringify(_offlineQueue));
      }
    } else {
      await idbSet(_OFFLINE_QUEUE_KEY, JSON.stringify(_offlineQueue));
    }
  } catch(e) {
    // FIX QUOTA : avec IndexedDB ce cas devient très rare (quota bien plus
    // large que localStorage), mais on garde l'alerte par prudence — un
    // appareil avec un disque quasi plein peut théoriquement encore échouer.
    console.error('[offline] ÉCHEC sauvegarde file offline (espace disque disponible insuffisant ?):', e);
    if (typeof notify === 'function') {
      notify('⚠️ Espace de stockage local insuffisant — une saisie récente risque de ne pas être conservée hors-ligne. Reconnectez-vous dès que possible.', 'err');
    }
  }
  _updateOfflineBadge();
}
function _updateOfflineBadge() {
  ...
  if (_offlineQueue.length > 0) {
    ...
    count.textContent = _offlineQueue.length + ' en attente';
  } else {
...
// ✅ FIX SACCADES CONNEXION INSTABLE : en Wi-Fi/4G instable, le navigateur
// déclenche 'online'/'offline' en rafale (plusieurs fois par seconde parfois),
// chaque déclenchement relançant _replayOfflineQueue(), un toast, et un
// renderPg() complet → c'est la cause principale de l'effet "saccadé" décrit.
// On débounce : on attend que le réseau se stabilise (300ms sans nouveau
// changement d'état) avant d'agir, et on ignore les déclenchements qui ne
// changent pas réellement l'état courant (ex: deux 'online' consécutifs).
const DELAY_NETWORK_FLAP_MS = 300;
let _networkFlapTimer = null;
let _lastAppliedOnlineState = null; // évite de ré-appliquer le même état en rafale

function _applyOnlineState(online) {
  if (_lastAppliedOnlineState === online) return; // déjà dans cet état, rien à refaire
  if (_lastAppliedOnlineState !== null) _noteNetworkTransition(); // ne compte pas le tout premier état au chargement
  _lastAppliedOnlineState = online;
  isOnline = online;
  const dot = document.querySelector('.online-dot');
  if (dot) dot.style.background = online ? 'var(--accent2)' : 'var(--danger)';
  setSyncStatus(online);
  if (online) {
    if (_offlineQueue.length > 0) _replayOfflineQueue();
  } else {
    notify('📵 Connexion perdue — mode hors-ligne activé');
  }
}

function _scheduleOnlineState(online) {
  if (_networkFlapTimer) clearTimeout(_networkFlapTimer);
  _networkFlapTimer = setTimeout(() => { _networkFlapTimer = null; _applyOnlineState(online); }, DELAY_NETWORK_FLAP_MS);
}

// Détection réseau en temps réel
window.addEventListener('online', () => { _scheduleOnlineState(true); });
window.addEventListener('offline', () => { _scheduleOnlineState(false); });
...
// ✅ FIX 4 : la file offline ne se déclenchait auparavant QUE si `isOnline`
// était déjà false (détecté via les événements navigateur online/offline).
// Or sur une connexion INSTABLE (pas coupée, juste dégradée), le navigateur
// reste souvent "online" alors que la requête Firestore échoue réellement
// (timeout, DNS, coupure momentanée) → l'écriture partait en erreur et
// n'était JAMAIS mise en file, donc potentiellement perdue pour l'utilisateur
// (ou nécessitait qu'il ressaisisse). On distingue maintenant les erreurs
// réseau transitoires (à mettre en file, retry automatique plus tard) des
// erreurs légitimes (droits insuffisants, données invalides — celles-là ne
// doivent PAS être mises en file car rejouer ne les résoudra jamais).
const _TRANSIENT_FIRESTORE_CODES = new Set([
  'unavailable', 'deadline-exceeded', 'internal', 'cancelled',
  'resource-exhausted', 'aborted', 'unknown'
]);
function _isTransientNetworkError(e) {
  if (!e) return false;
  if (e.code && _TRANSIENT_FIRESTORE_CODES.has(e.code)) return true;
  const msg = (e.message || '').toLowerCase();
  return msg.includes('network') || msg.includes('timeout') || msg.includes('failed to fetch') || msg.includes('offline');
}
...
async function fbAdd(col, data) {
  ...
  try {
    ...
    if (_isTransientNetworkError(e)) {
      // ✅ FIX 4 : connexion instable détectée pendant l'écriture → repli sur
      // la file offline au lieu de perdre la saisie ou de faire planter l'UI.
      console.warn('[fbAdd] Erreur réseau transitoire, mise en file offline :', e);
      const item = localAdd(col, data);
      await _enqueueOp('add', col, item);
      setSyncStatus(false);
      notify('📶 Connexion instable — enregistré localement, synchro automatique dès que possible', 'err');
      return item;
    }
    setSyncStatus(true);
    throw e;
...
async function fbUpdate(col, id, data) {
  ...
  try {
    ...
    if (_isTransientNetworkError(e)) {
      console.warn('[fbUpdate] Erreur réseau transitoire, mise en file offline :', e);
      localUpdate(col, id, data);
      await _enqueueOp('update', col, data, id);
      setSyncStatus(false);
      notify('📶 Connexion instable — modification enregistrée localement, synchro automatique dès que possible', 'err');
      return;
    }
    setSyncStatus(true);
    throw e;
...
async function fbDelete(col, id) {
  ...
  try {
    ...
    if (_isTransientNetworkError(e)) {
      console.warn('[fbDelete] Erreur réseau transitoire, mise en file offline :', e);
      localDelete(col, id);
      await _enqueueOp('delete', col, null, id);
      setSyncStatus(false);
      notify('📶 Connexion instable — suppression enregistrée localement, synchro automatique dès que possible', 'err');
      return;
    }
    setSyncStatus(true);
    throw e;
...
// FIX QUOTA : localStorage est plafonné à ~5-10 Mo par site (limite fixe du
// navigateur, non négociable). Le snapshot chiffré complet de la base (DB)
// et la file d'attente offline peuvent dépasser cette limite après quelques
// mois d'usage réel (calcul : une structure avec ~800 clients et leur
// historique de paiements peut déjà approcher les 10 Mo). IndexedDB n'a pas
// cette limite fixe — son quota dépend de l'espace disque réellement
// disponible sur l'appareil (généralement des centaines de Mo à plusieurs
// Go), ce qui élimine ce risque de perte silencieuse par saturation.
// Petite couche clé/valeur asynchrone, pour ne changer que le point de
// stockage sans toucher à la logique de chiffrement existante (qui continue
// à manipuler de simples chaînes en entrée/sortie, comme avec localStorage).
const _IDB_NAME    = 'mmb_local_store';
const _IDB_STORE   = 'kv';
...
// MIGRATION TRANSPARENTE : au tout premier lancement après mise à jour, si
// d'anciennes données existent encore dans localStorage (versions
// précédentes de l'app), on les recopie une fois vers IndexedDB avant de les
// supprimer de localStorage — aucune perte, aucune action requise côté
// utilisateur.
async function _migrateLocalStorageBlobToIdb(lsKey, idbKey) {
  try {
    const old = localStorage.getItem(lsKey);
    if (old === null) return;
    const existing = await idbGet(idbKey);
    if (existing === null) await idbSet(idbKey, old);
    localStorage.removeItem(lsKey);
    console.info('[migration] ' + lsKey + ' → IndexedDB (' + idbKey + ') effectuée.');
  } catch(e) {
    console.warn('[migration] Échec migration ' + lsKey + ' vers IndexedDB, les anciennes données restent en localStorage pour l\'instant:', e);
  }
}

async function idbGetRequestingSizeSafety(key) {
  // Sonde d'espace disque disponible, purement informative (n'empêche rien) —
  // permet de logger un avertissement précoce si l'appareil est vraiment
  // presque plein, avant d'atteindre une vraie erreur d'écriture.
  try {
    if (navigator.storage?.estimate) {
      const {usage, quota} = await navigator.storage.estimate();
      if (quota && usage / quota > 0.9) {
        console.warn('[stockage] Espace disque presque plein sur cet appareil (' + Math.round(usage/1024/1024) + ' Mo / ' + Math.round(quota/1024/1024) + ' Mo).');
      }
    }
  } catch(e) {}
}
...
async function loadLocalDataEncrypted() {
  try {
    // FIX QUOTA : migration transparente depuis localStorage si l'utilisateur
    // arrive d'une version antérieure de l'app.
    await _migrateLocalStorageBlobToIdb('gestcom_local_enc', 'gestcom_local_enc');
    const stored = await idbGet('gestcom_local_enc');
    if (stored) {
      const decrypted = await _decryptLocal(stored);
      if (decrypted) { DB = decrypted; }
    }
  } catch(e) { console.error('[offline] Échec lecture snapshot local (IndexedDB):', e); }
  if (!DB.mises) DB.mises = [];
  if (!DB.agences) DB.agences = [];
  if (!DB.primesPaliers) DB.primesPaliers = [];
  if (!DB.produits) DB.produits = [];
...
async function _doSaveLocalEncrypted() {
  ...
  try {
    ...
    // FIX QUOTA : IndexedDB au lieu de localStorage — plus de limite fixe
    // ~5-10 Mo, le snapshot complet de la base peut grossir sans risque de
    // saturation silencieuse (voir estimation faite avec l'utilisateur :
    // une structure de taille moyenne dépasse déjà 10 Mo après quelques mois).
    if (enc) await idbSet('gestcom_local_enc', enc);
    idbGetRequestingSizeSafety();
  } catch(e) { console.warn('saveLocalEncrypted: echec, donnees non persistees localement', e); } // FIX 3 : plus de fallback en clair
}
...
window.sauvegarderEditionClient = async function(){
  ...
  const newCommercialId = document.getElementById('edit-cl-com').value;
  // ── Contrôle avant départ : si le commercial du client change ici (mutation
  // manuelle via ce formulaire, pas seulement via l'outil de transfert en
  // masse), on note la date de ce changement. C'est cette date qui sert de
  // référence dans la section "Contrôle avant départ" pour ne compter que
  // les paiements reçus par le NOUVEAU commercial après sa prise de poste.
  const clAvant = DB.clients.find(c=>c._id===cid);
  const commercialAChange = clAvant && clAvant.commercialId !== newCommercialId;
  ...
  if(commercialAChange) patch.dateAffectation = TODAY;
  await fbUpdate('clients',cid,patch);
...
window.mcAjouterParCode = function(){
  ...
  // ✅ Un même produit peut désormais être ajouté plusieurs fois comme
  // lignes séparées du contrat (au lieu d'être automatiquement fusionné en
  // augmentant la quantité de la ligne existante).
...
window.mcAjouterArticle = function(){
  ...
  // ✅ Un même produit peut désormais être ajouté plusieurs fois comme
  // lignes séparées du contrat (au lieu d'être automatiquement fusionné en
  // augmentant la quantité de la ligne existante) — permet par exemple deux
  // lignes du même produit à des prix différents.
  _mcArticles.push({artId:sel.value, nom, qty:qtyVal, pv:pvVal});
  sel.value='';
...
// ✅ FIX VISIBILITÉ : badge dédié "Connexion instable" — distinct de
// synchronisé/hors-ligne. Si le réseau bascule plusieurs fois en peu de
// temps (flapping Wi-Fi/4G), on l'affiche pour rassurer l'utilisateur
// (les données restent en cache local, rien n'est perdu) plutôt que de le
// laisser voir l'app clignoter sans explication.
const UNSTABLE_WINDOW_MS = 15000;   // fenêtre d'observation
const UNSTABLE_THRESHOLD = 3;       // nb de bascules dans la fenêtre pour déclarer "instable"
const UNSTABLE_CLEAR_MS  = 10000;   // durée sans nouvelle bascule avant de considérer que c'est redevenu stable
let _networkTransitions = [];
let _unstableClearTimer = null;

function _showUnstableBadge(show) {
  const el = document.getElementById('unstable-badge');
  if (el) el.style.display = show ? 'flex' : 'none';
}

function _noteNetworkTransition() {
  const now = Date.now();
  _networkTransitions.push(now);
  _networkTransitions = _networkTransitions.filter(t => now - t <= UNSTABLE_WINDOW_MS);
  if (_networkTransitions.length >= UNSTABLE_THRESHOLD) {
    _showUnstableBadge(true);
    if (_unstableClearTimer) clearTimeout(_unstableClearTimer);
    _unstableClearTimer = setTimeout(() => {
      _networkTransitions = [];
      _showUnstableBadge(false);
    }, UNSTABLE_CLEAR_MS);
  }
}

let _lastSyncStatusOk = null;
function setSyncStatus(ok){
  if (_lastSyncStatusOk === ok) return; // ✅ FIX SACCADES : évite de réécrire le DOM si l'état est inchangé (appelé à chaque onSnapshot, potentiellement très fréquent en connexion instable)
  _lastSyncStatusOk = ok;
...
window.doLogin = async function(){
  ...
  if (session.role ===ROLES.COMMERCIAL) {
    ...
    // ⚠️ FIX PERTE SILENCIEUSE : si _loadOfflineQueue() a été appelée plus
    // tôt (ex: au boot, avant que la clé locale existe) et avait dû
    // reporter la lecture faute de clé, on la relit maintenant que
    // _localKey est prête — sans ça, une file chiffrée déjà présente sur
    // l'appareil pouvait rester invisible pour tout le reste de la session.
    if (_offlineQueueLoadPending) await _loadOfflineQueue();
  }
...
function verifierAlertesLivraison(){
  if(!session) return;
  // ✅ Les commerciaux ne doivent plus recevoir AUCUNE notification concernant
  // des clients (y compris leurs propres clients) — ces alertes de livraison
  // portent justement sur des clients, donc on coupe entièrement pour ce rôle.
  if(session.role === ROLES.COMMERCIAL) return;
  const clients = clientsDansAgence();
...
// ── Filet de sécurité (mobile) : vérification périodique côté serveur ──
// L'écouteur temps réel (_chatSubscribeConvList, ci-dessus) est censé capter
// toute nouvelle conversation instantanément. En pratique, sur mobile, le
// cache local Firestore met parfois du temps à "découvrir" un document qu'il
// n'a JAMAIS vu auparavant (typiquement : le tout premier message d'une
// personne avec qui on n'a jamais discuté), surtout sur connexion instable —
// contrairement à la mise à jour d'une conversation déjà connue, qui elle
// arrive normalement sans problème. On rattrape donc ce cas précis toutes les
// 20s (app au premier plan uniquement) avec une lecture qui CONTOURNE le
// cache local (getDocsFromServer) pour être sûr de refléter l'état réel.
let _chatPeriodicTimer = null;
function _chatStartPeriodicServerCheck(){
  if(_chatPeriodicTimer) return;
  // FIX COÛT FIREBASE : 20s → 3 minutes. C'est un simple filet de sécurité
  // de secours (le onSnapshot temps réel gère déjà la synchro normale) —
  // pas besoin d'une fréquence aussi élevée pour un rôle de rattrapage.
  _chatPeriodicTimer = setInterval(_chatServerCheckOnce, 180000);
}
async function _chatServerCheckOnce(){
  if(!db_fs || !session || document.hidden) return;
  try{
    const isAdminView = session.role === ROLES.ADMIN;
    const colRef = collection(db_fs,'chatConversations');
    const q = isAdminView ? colRef : query(colRef, where('participants','array-contains',session.userId));
    // ⚠️ FIX COÛT FIREBASE : getDocsFromServer() forçait une lecture facturée
    // en base à CHAQUE appel (toutes les 20s, par utilisateur connecté),
    // en ignorant volontairement le cache local — alors que le onSnapshot
    // temps réel actif en parallèle maintient déjà ce cache à jour sans
    // frais supplémentaire. Résultat mesuré : ~2,25M lectures/jour pour
    // seulement 15-50 utilisateurs (x300 par rapport à l'attendu), la
    // majorité venant très probablement de cette fonction.
    // getDocsFromCache() est GRATUITE (aucune lecture facturée) : elle lit
    // uniquement les données déjà synchronisées localement par le listener
    // temps réel, qui reste la véritable source de vérité comme le
    // rappelle le commentaire ci-dessous. En cas d'échec (cache vide,
    // listener pas encore initialisé), on retombe sur une vraie lecture
    // serveur classique (getDocs, qui respecte le cache Firestore standard,
    // contrairement à getDocsFromServer qui le contourne systématiquement).
    let snap;
    try {
    ...
    snap.forEach(d=>{
      const data = {...d.data(), _id:d.id};
      const isNewConv = !_chatConversations.has(d.id);
      _chatConversations.set(d.id, data);
      let hasUnread;
      if(isAdminView){
        const dernierMsg = data.updatedAt || 0;
        const vuLe = data.adminSeenAt?.[session.userId] || 0;
        hasUnread = !!(data.lastAuthorId && data.lastAuthorId !== session.userId && dernierMsg > vuLe);
      } else {
        hasUnread = (data.unread?.[session.userId] || 0) > 0;
      }
      if(hasUnread) totalUnread++;
      // Notifie seulement pour une conversation VRAIMENT nouvelle (jamais vue
      // avant sur cet appareil) que l'écouteur temps réel aurait manquée —
      // évite tout doublon avec la notification déjà gérée normalement par
      // onSnapshot pour les conversations déjà connues.
      if(isNewConv && hasUnread && data.lastAuthorId && data.lastAuthorId !== session.userId && d.id !== _chatConvId){
        const nomExp = data.names?.[data.lastAuthorId] || 'un utilisateur';
        playChatNotifSound();
    ...
    _chatUpdateBadge(totalUnread);
    if(_chatPanelOpen && !_chatConvId) renderChatConvList();
  }catch(e){ /* silencieux : simple filet de sécurité, la souscription temps réel reste la source principale */ }
}
...
// ── Reconnexion forcée au retour au premier plan (mobile) ──
// Sur mobile, le navigateur suspend le JS et coupe la connexion Firestore
// (WebSocket) dès que l'app passe en arrière-plan (écran verrouillé, onglet
// caché, app minimisée). Contrairement au desktop où l'onglet reste actif
// en continu, le badge de chat ne se met alors PAS à jour tant que l'app
// est en arrière-plan — c'est une limitation du système, pas un bug de
// l'app. En revanche, on peut garantir que dès que l'utilisateur revient
// sur l'app, le badge se rafraîchisse IMMÉDIATEMENT.
//
// ⚠️ Note technique : on NE PASSE PAS par enableNetwork(db_fs) ici. Cet
// appel peut entrer en collision avec la gestion interne du SDK Firestore
// lors des cycles arrière-plan/premier plan rapides sur mobile, et
// provoquer l'erreur "FIRESTORE INTERNAL ASSERTION FAILED: Unexpected
// state" (bug connu du SDK, cf. github.com/firebase/firebase-js-sdk).
// À la place, on désabonne puis ré-abonne proprement les listeners du
// chat : ça force un nouveau onSnapshot frais sans toucher à l'état
// réseau interne du SDK — beaucoup plus sûr.
let _chatForegroundResumeTimer = null;
function _chatForceResumeSync(){
  if (!db_fs || !session) return;
  if (_chatForegroundResumeTimer) clearTimeout(_chatForegroundResumeTimer);
  // Petit debounce pour éviter les appels multiples si plusieurs événements
  // (visibilitychange + focus + pageshow) se déclenchent presque en même temps.
  _chatForegroundResumeTimer = setTimeout(() => {
    // Re-souscrit la liste des conversations (alimente le badge).
    const oldConv = _activeListeners.get('chat_conv_list');
    if (oldConv) { try{ oldConv(); }catch(_){} _activeListeners.delete('chat_conv_list'); }
    _chatConvListReady = false;
    _chatSubscribeConvList();
    // Si une conversation est ouverte, re-souscrit aussi ses messages/typing.
    if (_chatConvId) {
      _chatSubscribeMessages(_chatConvId);
      _chatSubscribeTyping(_chatConvId);
    }
  }, 50);
}
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') _chatForceResumeSync();
});
window.addEventListener('focus', _chatForceResumeSync);
// pageshow avec persisted=true couvre le cas iOS Safari où la page est
// restaurée depuis le cache (bfcache) sans re-déclencher 'load'.
window.addEventListener('pageshow', (e) => { if (e.persisted) _chatForceResumeSync(); });
...
function renderChatMessages(){
  ...
  el.innerHTML = _chatMessages.map(m=>{
    ...
    const contenu = m.type === 'image'
      ...
      : m.type === 'recu'
      ? `<div class="chat-msg-recu">
          <div style="font-size:13px;">${esc(m.texte||'📄 Reçu')}</div>
          <button class="btn btn-xs" style="margin-top:8px;background:rgba(201,168,76,0.15);color:var(--accent);border:1px solid rgba(201,168,76,0.35);font-weight:700;" onclick="ouvrirRecuDepuisChat('${esc(m.recuId)}')">🖨️ Ouvrir / Imprimer le reçu</button>
        </div>`
      : esc(m.texte||'');
...
const BN_CONFIG = {
  ...
  commercial: {
    ...
    drawer: [
      {id:'historique', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>', label:'Mon historique'},
      {id:'produits', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>', label:'Produits'},
      {id:'saisie-adhesions', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><polyline points="16 11 18 13 22 9"/></svg>', label:'Adhésions'},
      {id:'__bt_printer__', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6.5 6.5 17.5 17.5 12 23 12 1 17.5 6.5 6.5 17.5"/></svg>', label:'Imprimante BT', special:'bt'},
...
window.go = async function(id){ // ✅ FIX : async pour await setupPageListeners et capturer les erreurs
  ...
  if(!isControleur && BTN[id]){
    ...
    else if(id === 'produits') showBtn = ['admin','gestionnaire_stock'].includes(role);
...
function renderPg(id){
  // ── FIX « ÉCRAN QUI VA ET VIENT » ──
  // renderPg() réécrit le HTML de la page (innerHTML), ce qui remet
  // instinctivement le conteneur scrollable #main tout en haut. Résultat :
  // dès qu'un tiers (n'importe quel utilisateur) modifie une donnée dont
  // dépend la page affichée, un rafraîchissement automatique se déclenche et
  // fait "sauter" l'écran, même en pleine lecture/scroll. On capture donc la
  // position de défilement juste avant, et on la restaure juste après —
  // le contenu se met à jour, mais l'écran, lui, ne bouge plus.
  const _mainEl = document.getElementById('main');
  const _scrollTop = _mainEl ? _mainEl.scrollTop : 0;
  const _scrollLeft = _mainEl ? _mainEl.scrollLeft : 0;
  ...
  try {
    ...
    // Restaure la position de scroll capturée en haut de la fonction — le
    // contenu vient d'être régénéré, mais l'écran reste visuellement stable.
    // requestAnimationFrame : laisse le DOM fraîchement injecté se poser
    // avant de réappliquer le scroll (sinon la hauteur peut ne pas encore
    // être à jour et la restauration serait ignorée par le navigateur).
    if (_mainEl && (_scrollTop || _scrollLeft)) {
      _mainEl.scrollTop = _scrollTop;
      _mainEl.scrollLeft = _scrollLeft;
      requestAnimationFrame(()=>{ _mainEl.scrollTop = _scrollTop; _mainEl.scrollLeft = _scrollLeft; });
    }
  }
}
...
// ✅ FIX ÉCONOMIE LECTURES : fenêtre glissante de paiements (366 jours,
// ou plus si dashPeriod est réglé plus large) déjà récupérée par une
// requête ciblée dans renderAdminDash() (_fetchPaiementsDateRange), et
// réutilisée ici par les graphiques au lieu de recharger toute la
// collection 'paiements' (potentiellement des dizaines de milliers de
// documents) via un abonnement onSnapshot permanent sur la collection
// entière. Alimentée uniquement par renderAdminDash().
let _dashPaysFenetre = [];

// Retourne les paiements (fenêtre glissante récente) filtrés selon l'agence
// du dashboard — ne représente PAS l'historique complet, uniquement les
// ~366 derniers jours, ce qui suffit largement pour les graphiques du
// tableau de bord (barres 12 mois, courbe de collecte, donut par commercial).
function dashPays(){
  if(!dashAgenceFilter) return _dashPaysFenetre;
  const comIds = DB.commerciaux.filter(c=>c.agenceId===dashAgenceFilter).map(c=>c._id);
  return _dashPaysFenetre.filter(p=>comIds.includes(p.commercialId));
}
...
async function renderAdminDash(){
  ...
  // Alimente la fenêtre glissante utilisée par dashPays() pour les graphiques
  // (barres 12 mois, courbe de collecte, donut par commercial) — évite que
  // ces fonctions rechargent toute la collection 'paiements'.
  _dashPaysFenetre = moisPaysServer;
...
function renderProduits() {
  const canEdit = session && ['admin','gestionnaire_stock'].includes(session.role);
...
window.saveProduit = async function() {
  if (!session || !['admin','gestionnaire_stock'].includes(session.role)) { notify('Accès refusé','err'); return; }
...
function renderStock(){
  ...
  renderInventaire();
  // ✅ Générateur "Valeur des entrées" réservé à l'admin.
  const veWrap = document.getElementById('valeur-entrees-wrap');
  if(veWrap){
    veWrap.style.display = session?.role==='admin' ? '' : 'none';
...
window.renderStockMvts = function(){
  ...
  document.getElementById('tb-stock-mvt').innerHTML=mvts.map(m=>{
    ...
    return`<tr>
      ...
      <td>${session?.role==='admin'?`<button class="btn btn-xs btn-warn" onclick="deleteStockMvt('${m._id}')" title="Supprimer ce mouvement et corriger le stock en conséquence">🗑</button>`:''}</td>
    </tr>`;
  }).join('')||'<tr><td colspan="8" class="emp">Aucun mouvement</td></tr>';
};

// ─── Suppression d'un mouvement de stock (admin uniquement) ───
// Corrige le stock courant de l'article en sens inverse du mouvement
// supprimé (une entrée supprimée retire la quantité, une sortie supprimée
// la restitue), puis supprime l'enregistrement du mouvement lui-même.
// ⚠️ Ne recalcule PAS rétroactivement le "stockApres" des mouvements
// suivants pour cet article : ces valeurs restent des photos historiques
// de l'état au moment de chaque mouvement, seul le stock courant de
// l'article (source de vérité pour les opérations futures) est corrigé.
window.deleteStockMvt = async function(mvtId){
  if(session?.role!=='admin'){ notify('Accès réservé à l\'administrateur','err'); return; }
  const m = DB.stockMvts.find(x=>x._id===mvtId);
  if(!m){ notify('Mouvement introuvable','err'); return; }
  const art = DB.articles.find(a=>a._id===m.articleId);
  const ok = await confirmDialog(
    `Supprimer ce mouvement (${m.type==='entree'?'Entrée':'Sortie'} de ${m.qty} ${art?esc(art.unite):''} — ${art?esc(art.nom):'article inconnu'}) ?<br><br>Le stock actuel de l'article sera automatiquement corrigé en conséquence.`,
    {title:'🗑 Supprimer le mouvement', okLabel:'Supprimer', danger:true}
  );
  if(!ok) return;
  if(art){
    // Sens inverse du mouvement : une entrée supprimée retire la quantité
    // du stock, une sortie supprimée la restitue.
    const nouveauStock = art.stock + (m.type==='entree' ? -m.qty : m.qty);
    await fbUpdate('articles', art._id, {stock: nouveauStock});
  }
  await fbDelete('stockMvts', mvtId);
  notify('Mouvement supprimé et stock corrigé ✓');
  renderStockMvts();
  renderInventaire();
};

// ─── Générateur : valeur totale des entrées d'un jour (prix d'achat) ───
// Réservé à l'admin. Calcule, pour une date donnée, la somme des quantités
// entrées (mouvements de type 'entree') multipliées par le prix d'achat
// (a.pa) de chaque article — donne la valeur d'achat totale réceptionnée
// ce jour-là, article par article et en cumul.
window.calculerValeurEntrees = function(){
  if(session?.role!=='admin'){ notify('Accès réservé à l\'administrateur','err'); return; }
  const date = document.getElementById('ve-date')?.value;
  if(!date){ notify('Sélectionnez une date','err'); return; }
  const entrees = (DB.stockMvts||[]).filter(m=>m.type==='entree' && m.date===date);
  // Cumul par article (plusieurs entrées du même article le même jour possibles)
  const parArticle = {};
  entrees.forEach(m=>{
    if(!parArticle[m.articleId]) parArticle[m.articleId] = 0;
    parArticle[m.articleId] += (Number(m.qty)||0);
  });
  const tbody = document.getElementById('tb-valeur-entrees');
  const totalEl = document.getElementById('ve-total');
  const artIds = Object.keys(parArticle);
  if(!artIds.length){
    if(tbody) tbody.innerHTML = '<tr><td colspan="4" class="emp">Aucune entrée de stock à cette date.</td></tr>';
    if(totalEl) totalEl.textContent = 'Total : ' + fmt(0);
    return;
  }
  let totalGeneral = 0;
  const rows = artIds.map(aid=>{
    const a = getArt(aid);
    const qty = parArticle[aid];
    const pa = Number(a.pa)||0;
    const valeur = qty * pa;
    totalGeneral += valeur;
    return `<tr>
      <td class="fw6">${esc(a.nom)}</td>
      <td>${qty} ${esc(a.unite||'pcs')}</td>
      <td>${pa>0?fmt(pa):'<span style="color:var(--warn)">Non défini</span>'}</td>
      <td style="font-weight:700;color:var(--accent2);">${fmt(valeur)}</td>
    </tr>`;
  }).join('');
  if(tbody) tbody.innerHTML = rows;
  if(totalEl) totalEl.textContent = 'Total : ' + fmt(totalGeneral);
};
...
window.renderInventaire = function(){
  ...
  if(!tbody) return;
  // Total entré / sorti par article, calculé depuis l'historique des
  // mouvements de stock (indépendant du stock courant, qui peut avoir été
  // corrigé manuellement — ces totaux reflètent le cumul réel des mouvements).
  const totauxParArticle = {};
  (DB.stockMvts||[]).forEach(m=>{
    if(!totauxParArticle[m.articleId]) totauxParArticle[m.articleId] = {entree:0, sortie:0};
    if(m.type==='entree') totauxParArticle[m.articleId].entree += (Number(m.qty)||0);
    else if(m.type==='sortie') totauxParArticle[m.articleId].sortie += (Number(m.qty)||0);
  });
  tbody.innerHTML = arts.sort((a,b)=>(a.nom||'').localeCompare(b.nom||'')).map(a=>{
    ...
    const tot = totauxParArticle[a._id] || {entree:0, sortie:0};
    return `<tr>
      <td><span class="tag">${esc(a.ref||'—')}</span></td>
      <td class="fw6">${esc(a.nom)}</td>
      ...
      <td style="font-weight:700;color:var(--accent2);">+${tot.entree}</td>
      <td style="font-weight:700;color:var(--danger);">-${tot.sortie}</td>
      <td>${statut}</td>
    </tr>`;
  }).join('')||'<tr><td colspan="9" class="emp">Aucun article trouvé.</td></tr>';
};
...
function renderLivraisons(){
  ...
  // ── Tableau ──
  // FIX PERF : sans filtre de date, "livs" peut contenir l'historique complet
  // des livraisons depuis le début de l'activité. On plafonne le rendu DOM
  // (les KPI ci-dessus restent calculés sur la liste complète filtrée).
  const livsTronquees = livs.length > _HIST_RENDER_CAP;
  const livsAffichees = livsTronquees ? livs.slice(0, _HIST_RENDER_CAP) : livs;
  const _bandeauTronqueLiv = livsTronquees
    ? `<tr><td colspan="8" style="text-align:center;padding:10px;font-size:11px;color:var(--muted);background:rgba(247,201,79,0.08);">⚠️ Affichage limité aux ${_HIST_RENDER_CAP} livraisons les plus récentes sur ${livs.length} — filtrez par date pour voir le reste.</td></tr>`
    : '';
  document.getElementById('tb-livraisons').innerHTML = _bandeauTronqueLiv + livsAffichees.map(l=>{
...
async function renderControleDepart(){
  ...
  if(!sel || !sel.value){
    if(zone) zone.innerHTML = '<div class="emp" style="padding:40px;">Sélectionnez un commercial pour voir la liste des clients ayant payé avec lui depuis sa prise de poste (date de sa dernière affectation).</div>';
  ...
  if(zone) zone.innerHTML = '<div class="emp" style="padding:40px;">⏳ Recherche des paiements enregistrés par ce commercial depuis sa prise de poste…</div>';
  ...
  // ── Filtre "après mutation" ──
  // Pour chaque client, dateAffectation = date à laquelle CE commercial a
  // pris le client en charge (enregistrée automatiquement lors d'un transfert
  // ou d'un changement de commercial via la fiche client). Seuls les
  // paiements reçus par ce commercial À PARTIR de cette date comptent : ça
  // permet de vérifier qu'un commercial nouvellement affecté à un marché
  // encaisse bien auprès des clients qui lui ont été confiés, sans compter
  // d'anciens paiements hérités d'avant sa prise de poste. Si le client n'a
  // pas de dateAffectation enregistrée (jamais transféré, avec ce commercial
  // depuis toujours), tous ses paiements comptent normalement.
  const paysFiltres = paysCom.filter(p=>{
    if(!p.clientId) return false;
    const cl = getCl(p.clientId);
    if(!cl || !cl.dateAffectation) return true;
    return (p.date||'') >= cl.dateAffectation;
  });

  // Regrouper par client : nb paiements, total payé À CE commercial (depuis sa prise de poste), dernière date
  const parClient = new Map();
  for(const p of paysFiltres){
  ...
  const rows = [...parClient.entries()].map(([clientId, agg])=>{
    ...
    return {
      ...
      total: agg.total,
      dernier: agg.dernier,
      depuis: cl?.dateAffectation || null,
  ...
  if(summary){
    ...
    summary.innerHTML = `
      <div class="kpi-card kc-blue"><div class="kpi-lbl">Clients ayant payé depuis sa prise de poste</div><div class="kpi-val kv-blue">${nbClients}</div></div>
  ...
  if(zone){
    zone.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px;">
        <div style="font-size:12.5px;color:var(--muted);">Résultat pour <strong style="color:var(--text);">${esc(com?.nom||'?')}</strong> — ${nbClients} client(s) ayant payé au moins une fois depuis sa prise de poste (triés par activité la plus récente)</div>
      ...
      ${rows.map(r=>`<tr>
        ...
        <td style="font-size:11px;">${esc(r.depuis||'—')}</td>
...
window.renderControleDepart = renderControleDepart;
...
window.exportControleDepart = function(){
  ...
  const cols = ['Client','Code client','Téléphone','Nb paiements','Total payé (avec lui)','Depuis (mutation)','Dernier paiement','Statut contrat','Assignation actuelle'];
  const data = _cdLastRows.map(r=>[
    r.nom, r.code||'', r.tel, r.nb, r.total, r.depuis||'', r.dernier||'',
...
let _histLastSig = null; // signature (date|commercial) du dernier contexte réellement affiché — voir renderHist()
async function renderHist(){
  const el = document.getElementById('h-date');
  const fd = el ? el.value : TODAY;
  // ── Commercial : historique personnel uniquement ──
  // DB.paiements est déjà restreint côté serveur à SES SEULS paiements pour
  // ce rôle (requête Firestore where commercialId==uid) — il n'y a donc
  // techniquement rien à filtrer en plus ici. On masque simplement le
  // sélecteur "commercial" (inutile, il n'y a jamais que lui) et la colonne
  // "Commercial" du tableau (toujours son propre nom, redondant).
  const isCommercialSelf = session && session.role === ROLES.COMMERCIAL;
  const comSelEl = document.getElementById('h-filter-com');
  if(comSelEl) comSelEl.style.display = isCommercialSelf ? 'none' : '';
  const comColTh = document.getElementById('h-com-col-th');
  if(comColTh) comColTh.style.display = isCommercialSelf ? 'none' : '';
  ...
  const comSel = isCommercialSelf ? null : comSelEl;
  if(comSel){
    const comsVisibles = comsDansAgence().filter(c=>c.role===ROLES.COMMERCIAL);
    const comsSig = comsVisibles.map(c=>c._id).join(',');
  ...
  // ── Même correctif que registre/fiche : ne montrer "Chargement…" que lors
  // d'un vrai changement de contexte (date/commercial/recherche), pas à
  // chaque rafraîchissement silencieux déclenché par une donnée modifiée
  // par quelqu'un d'autre — sinon l'écran clignote en pleine consultation.
  const _histSig = fd+'|'+filtreComId;
  const _isHistContextChange = _histLastSig !== _histSig;
  _histLastSig = _histSig;
  if (fd) {
    if (tbHist && _isHistContextChange) tbHist.innerHTML = `<tr><td colspan="8" class="emp">⏳ Chargement…</td></tr>`;
    try {
      await Promise.all([
  ...
  document.getElementById('tb-hist').innerHTML=pays.map(p=>{
    ...
    const btnSuppr = isAdminHist
      ...
      : '';
    const colCom = isCommercialSelf ? '' : `<td><span class="tag">${esc(com.nom)}</span></td>`;
    ...
    ${colCom}${btnSuppr}</tr>`;
  }).join('')||`<tr><td colspan="${isAdminHist?8:(isCommercialSelf?6:7)}" class="emp">${fd?'Aucun paiement ce jour':'Aucun paiement enregistr&eacute;'}</td></tr>`;
}
...
async function renderRegistre(){
  ...
  // ── Même correctif que renderFiche() : ne montrer "Chargement…" que lors
  // d'un vrai changement de contexte (date ou agence), pas à chaque
  // rafraîchissement silencieux déclenché par une donnée modifiée par
  // quelqu'un d'autre — sinon l'écran clignote en pleine consultation.
  const _regSig = date+'|'+(effectiveAgenceId||'');
  const _isRegContextChange = _regLastSig !== _regSig;
  if (contentEl && _isRegContextChange) contentEl.innerHTML = '<div class="emp" style="padding:50px">⏳ Chargement des données du jour…</div>';
  _regLastSig = _regSig;
  ...
  const comsData = coms.map(c=>{
    ...
    // Nouveaux clients enregistrés CE jour par ce commercial — condition de
    // validation de la prime (au moins 2 requis), même logique que
    // renderFiche() : (c.debut===date || c.createdAt===date), hors imports.
    const nbNouveauxClients = DB.clients.filter(cl=>
      cl.commercialId===c._id &&
      (cl.debut===date || cl.createdAt===date) &&
      cl.origine!=='import'
    ).length;
    return{
      com: c,
  ...
  // ── Calcul total primes à déduire ──
  // Base = uniquement les cotisations (hors adhésions/rachats), et prime
  // validée seulement si ≥2 nouveaux clients enregistrés ce jour.
  const totalPrimes = comsData.reduce((a,d)=>{ const p=calculerPrime(d.montantCotis, d.nbNouveauxClients); return a+(p?Number(p.montant):0); },0);
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>
        ${comsData.map((d,i)=>{
          ...
          const prime = calculerPrime(d.montantCotis, d.nbNouveauxClients);
          const medal = prime ? (d.montantCotis >= 150000 ? '🥇' : d.montantCotis >= 80000 ? '🥈' : '🥉') : '';
      ...
      <div style="border-top:1px solid var(--border);padding-top:12px;">
        ...
        ${membres.length===0?'<div class="emp" style="padding:10px;font-size:11px;">Aucun membre assigné</div>':
          membres.map(m=>`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.04);">
            ...
            ${session?.role==='admin'&&m.email?`<button class="btn btn-ghost btn-xs" style="flex-shrink:0;" title="Envoyer un email de réinitialisation du mot de passe à ${esc(m.email)}" onclick="resetPasswordCom('${m._id}')">🔑</button>`:''}
          </div>`).join('')}
      </div>
...
window.confirmerTransfert = async function(){
  ...
  for(const cid of selected){
    try{ await fbUpdate('clients', cid, {commercialId: destId, dateAffectation: TODAY}); ok++; }
    catch(e){ err++; }
  }
  closeM('m-transfert');
...
async function renderFiche(){
  ...
  // ── FIX « ÉCRAN QUI VA ET VIENT » (fiche du jour) ──
  // Avant : le contenu était vidé et remplacé par "⏳ Chargement…" à CHAQUE
  // appel de renderFiche(), y compris les rafraîchissements automatiques
  // silencieux déclenchés par une modification de données de N'IMPORTE QUI
  // dans le système (via _debouncedRenderPg) — même quand la date et le
  // commercial affichés n'avaient pas changé. Résultat : la page clignotait
  // en plein milieu de la consultation. On ne montre maintenant ce message
  // que lors d'un vrai changement de contexte (date différente, commercial
  // différent, ou tout premier chargement) ; un simple rafraîchissement en
  // arrière-plan met à jour le contenu sans passer par un écran vide.
  const _ficheSig = fd+'|'+comId;
  const _isFicheContextChange = _ficheLastSig !== _ficheSig;
  if (ficheContentEl && _isFicheContextChange) ficheContentEl.innerHTML = '<div class="emp" style="padding:40px">⏳ Chargement des données du jour…</div>';
  _ficheLastSig = _ficheSig;
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:11px;padding:20px;">
      ...
      <div class="tw" style="margin-bottom:16px;"><table>
        ...
        <tbody>${clientsPayeurs.map(c=>{
          const s=stats(c);
          // ── Cumul des mises multiples du même client le même jour ──
          // Avant : pays.find(...) ne prenait que le 1er paiement du jour pour
          // ce client, ignorant les mises suivantes (montant/ratio sous-évalués
          // si le client paie plusieurs fois dans la journée). On agrège
          // maintenant TOUS les paiements du jour pour ce client.
          const paysClientJour = pays.filter(p=>p.clientId===c._id)
            .slice().sort((a,b)=>(a.heure||'').localeCompare(b.heure||''));
          const montantCumule = paysClientJour.reduce((a,p)=>a+(p.montant||0),0);
          const derniereMise = paysClientJour[paysClientJour.length-1] || {};
          const nbMises = paysClientJour.length;
          // Niveau : joursCouv actuel du client (déjà basé sur TOUT l'historique
          // de paiements via stats()/_payIndex(), donc reflète nativement le
          // DERNIER niveau de cotisation, mises du jour comprises).
          ...
          const heureLabel = (nbMises>1)
            ? paysClientJour.map(p=>p.heure||'—').join(', ')+' <span title="'+nbMises+' mises cumulées ce jour" style="display:inline-flex;align-items:center;margin-left:4px;background:rgba(100,160,247,0.15);border:1px solid rgba(100,160,247,0.4);border-radius:10px;padding:1px 7px;font-size:9px;font-weight:700;color:#64a0f7;white-space:nowrap;">×'+nbMises+' mises</span>'
            : (derniereMise.heure||'—')+badgeCorrection(derniereMise);
          return '<tr>'
            ...
            +'<td style="color:var(--muted);font-size:11px">'+heureLabel+'</td>'
            +'<td style="font-weight:700;color:var(--accent2)">'+fmt(montantCumule)+'</td>'
            +'<td>'+ratio(montantCumule,s.m)+'</td>'
...
// Retourne le palier le plus élevé atteint pour un montant de COTISATIONS
// donné, à condition que le commercial ait enregistré au moins 2 nouveaux
// clients ce même jour. Sans ces 2 nouveaux clients, aucune prime n'est
// accordée même si le montant du palier est atteint.
function calculerPrime(montantCotis, nbNouveauxClients) {
  if (Number(nbNouveauxClients || 0) < 2) return null;
  const paliers = (DB.primesPaliers || [])
    .filter(p => montantCotis >= Number(p.seuil))
...
window.savePay = async function(){
  ...
  try{
    ...
    notify(estCorrection
      ...
      : `Paiement de ${fmt(amt)} enregistré pour ${esc(c.nom)} ✓`);
...
// ─── Réinitialisation du mot de passe d'un utilisateur (admin uniquement) ───
// L'admin déclenche l'envoi d'un email de réinitialisation Firebase Auth
// standard à la place de l'utilisateur — utile car les commerciaux n'ont
// pas forcément un accès email fiable/immédiat pour faire la démarche
// eux-mêmes depuis l'écran de connexion.
window.resetPasswordCom = async function(id){
  if(session?.role!=='admin'){ notify('Accès réservé à l\'administrateur','err'); return; }
  const m = DB.commerciaux.find(c=>c._id===id);
  if(!m){ notify('Utilisateur introuvable','err'); return; }
  if(!m.email){ notify('Cet utilisateur n\'a pas d\'email enregistré — réinitialisation impossible','err'); return; }
  const ok = await confirmDialog(
    `Envoyer un email de réinitialisation du mot de passe à <strong>${esc(m.nom)}</strong> (${esc(m.email)}) ?`,
    {title:'🔑 Réinitialiser le mot de passe', okLabel:'Envoyer', danger:false}
  );
  if(!ok) return;
  try{
    await sendPasswordResetEmail(auth, m.email);
    notify(`✉️ Email de réinitialisation envoyé à ${m.email}`);
  }catch(e){
    console.error('Échec envoi réinitialisation mot de passe:', e);
    // Message adapté selon le code d'erreur Firebase le plus courant
    const msg = e?.code === 'auth/user-not-found'
      ? `Aucun compte Firebase Auth trouvé pour ${m.email} — vérifiez qu'il a bien été créé dans la Console Firebase.`
...
window.editArt = function(id){
  ...
  renderArtGalleryEdit();
  openM('m-article', {skipReset:true});
};
...
// ========= MODALS =========
window.openM = function(id, opts={}){
  document.getElementById(id).classList.add('open');
  // ✅ FIX : quand la modale est ouverte pour ÉDITER un élément existant
  // (ex: editArt() qui vient de pré-remplir le formulaire), on ne doit PAS
  // réinitialiser les champs juste après — sinon l'ID d'édition (ex:
  // 'art-edit-id') repart à vide et la sauvegarde crée un nouvel élément
  // au lieu de mettre à jour l'existant. `opts.skipReset` permet aux
  // fonctions d'édition de garder les valeurs qu'elles viennent de poser.
...
// ── Filtrage par rôle : un commercial ne doit voir/entendre que ce qui le
// concerne (messages chat, ses actions réussies, ses erreurs) — pas le
// "bruit" technique (synchronisation, session, accès refusé à des pages
// qu'il n'a de toute façon pas). Les notify() étant toujours déclenchés
// localement par les actions de l'utilisateur courant (pas de diffusion
// entre commerciaux), un simple filtre par mots-clés suffit : pas besoin de
// filtrer par commercialId, chaque commercial ne voit déjà que ses propres
// événements dans son propre navigateur.
const NOTIF_BELL_HIDDEN_FOR_COMMERCIAL = [
  /Synchronisation/i, /hors-ligne/i, /connexion perdue/i,
  /Session (expirée|inactive|invalide|révoquée)/i, /veuillez vous reconnecter/i,
  /Accès (non autorisé|refusé)/i, /Base de données initialisée/i,
  /données sont chargées/i,
  // ✅ À la demande : les commerciaux ne doivent plus voir AUCUNE
  // notification concernant des clients — même leurs propres clients
  // (création, mise à jour, suppression, transfert, contrôle, livraison
  // bloquée par solde insuffisant, etc.). Filtre volontairement large sur
  // le mot "client" plutôt qu'une liste de cas particuliers, pour rester
  // valable même si de nouveaux messages liés aux clients sont ajoutés
  // ailleurs dans l'app à l'avenir.
  /client/i
];
function _notifBellVisibleForCurrentRole(it){
  if(!session || session.role !== ROLES.COMMERCIAL) return true;
  return !NOTIF_BELL_HIDDEN_FOR_COMMERCIAL.some(re=>re.test(it.msg));
}
function _notifBellVisibleItems(){
  return _notifBellItems.filter(_notifBellVisibleForCurrentRole);
}

// ── Son générique pour toute nouvelle notification de la cloche ──
// (Le chat a déjà son propre son via playChatNotifSound(), appelé avant
// notify() dans les listeners de messages — on évite donc de le rejouer ici
// pour ne pas avoir un double bip sur un même message.)
function playBellNotifSound(){
  try{
    if(!_chatAudioCtx) _chatAudioCtx = new (window.AudioContext||window.webkitAudioContext)();
    const ctx = _chatAudioCtx;
    if(ctx.state === 'suspended') ctx.resume();
  ...
  }catch(e){ /* AudioContext indisponible : silencieux, sans conséquence */ }
}

function _notifBellPush(msg,t,clientId=null){
  const it = { id: Date.now()+'_'+Math.random().toString(36).slice(2,7), msg, t, ts: Date.now(), read:false, clientId };
  _notifBellItems.unshift(it);
  ...
  // Bip uniquement si l'item est effectivement visible pour le rôle courant
  // (pas de bruit sonore pour un événement masqué), et pas pour les messages
  // de chat qui ont déjà joué leur propre son juste avant.
  if(!msg.startsWith('💬') && _notifBellVisibleForCurrentRole(it)) playBellNotifSound();
}
function _notifBellUnreadCount(){
  return _notifBellVisibleItems().reduce((n,it)=>n+(it.read?0:1),0);
}
...
function _notifBellRender(){
  ...
  if(!list) return;
  const items = _notifBellVisibleItems();
  if(!items.length){
...
// ── Visibilité de la cloche 🔔 : secrétaire, chef d'agence, admin — et
// désormais commercial (avec un contenu filtré, voir _notifBellVisibleForCurrentRole) ──
const NOTIF_BELL_ALLOWED_ROLES = [ROLES.SECRETAIRE, ROLES.CHEF_AGENCE, ROLES.ADMIN, ROLES.COMMERCIAL];
...
// ── Cloche déplaçable (glisser-déposer) ──
// Position sauvegardée par utilisateur (localStorage), en left/top explicites
// (on convertit une fois pour toutes depuis le right/bottom par défaut du
// CSS). Fonctionne au doigt (mobile) comme à la souris via Pointer Events.
function _notifBellPosKey(){
  return 'notifBellPos:' + (session?.userId || 'anon');
}
function _notifBellInitDrag(){
  const bell = document.getElementById('notif-bell');
  if(!bell || bell._dragInit) return;
  bell._dragInit = true;

  // Applique une position sauvegardée si elle existe.
  try{
    const saved = JSON.parse(localStorage.getItem(_notifBellPosKey())||'null');
    if(saved && typeof saved.left==='number' && typeof saved.top==='number'){
      _notifBellApplyPos(saved.left, saved.top);
    }
  }catch(e){}
  ...
  const onPointerDown = (e)=>{
    // Clic droit / multi-touch ignorés.
  ...
  const onPointerUp = (e)=>{
    ...
    if(moved){
      // Sauvegarde la position finale, et empêche le prochain "click" natif
      // (généré après pointerup) d'ouvrir le panneau — on vient de glisser,
      // pas de cliquer.
      const rect = bell.getBoundingClientRect();
      try{ localStorage.setItem(_notifBellPosKey(), JSON.stringify({left:rect.left, top:rect.top})); }catch(e){}
      bell._suppressNextClick = true;
    }
  };
  bell.addEventListener('pointerdown', onPointerDown);
  window.addEventListener('pointermove', onPointerMove);
  window.addEventListener('pointerup', onPointerUp);
  // Recadre la cloche si elle sort de l'écran après une rotation / resize.
  window.addEventListener('resize', ()=>{
    const rect = bell.getBoundingClientRect();
    const left = Math.min(rect.left, window.innerWidth - bell.offsetWidth - 4);
    const top  = Math.min(rect.top,  window.innerHeight - bell.offsetHeight - 4);
    if(left!==rect.left || top!==rect.top) _notifBellApplyPos(Math.max(4,left), Math.max(4,top));
  });
}
function _notifBellApplyPos(left, top){
  const bell = document.getElementById('notif-bell');
  if(!bell) return;
...
// Intercepte le click natif juste après un glisser, pour ne pas ouvrir le
// panneau par erreur en fin de déplacement (voir onclick="notifBellToggle()" en HTML).
const _origNotifBellToggle = window.notifBellToggle;
window.notifBellToggle = function(force){
  const bell = document.getElementById('notif-bell');
  if(bell && bell._suppressNextClick){
    bell._suppressNextClick = false;
    return; // ce "click" est la conséquence d'un glisser-déposer, pas un vrai clic
  }
  return _origNotifBellToggle(force);
};
...
window.depGenererPrimesJour = async function(){
  ...
  try {
    ...
    for(const c of coms){
      ...
      // Nouveaux clients enregistrés ce jour par ce commercial — condition
      // de validation de la prime (au moins 2 requis).
      const nbNouveauxClients = DB.clients.filter(cl=>
        cl.commercialId===c._id &&
        (cl.debut===date || cl.createdAt===date) &&
        cl.origine!=='import'
      ...
      const palier = calculerPrime(montantCotis, nbNouveauxClients);
...
function renderDepensesCommerciaux(){
  ...
  // FIX PERF : idem reçus — on plafonne le rendu DOM, pas les données/totaux.
  const depTronquees = depenses.length > _HIST_RENDER_CAP;
  const depAffichees = depTronquees ? depenses.slice(0, _HIST_RENDER_CAP) : depenses;
  const _bandeauTronqueDep = depTronquees
    ? `<tr><td colspan="7" style="text-align:center;padding:10px;font-size:11px;color:var(--muted);background:rgba(247,201,79,0.08);">⚠️ Affichage limité aux ${_HIST_RENDER_CAP} dépenses les plus récentes sur ${depenses.length} — affinez les filtres (date, commercial) pour voir le reste.</td></tr>`
    : '';
  tbody.innerHTML = _bandeauTronqueDep + depAffichees.map(d=>`<tr>
...
function renderHistoriqueRecus(){
  ...
  // FIX PERF : on ne rend que les _HIST_RENDER_CAP reçus les plus récents ;
  // le total affiché juste au-dessus reste calculé sur la liste complète.
  const recusTronques = recus.length > _HIST_RENDER_CAP;
  const recusAffiches = recusTronques ? recus.slice(0, _HIST_RENDER_CAP) : recus;
  const _bandeauTronque = recusTronques
    ? `<tr><td colspan="8" style="text-align:center;padding:10px;font-size:11px;color:var(--muted);background:rgba(247,201,79,0.08);">⚠️ Affichage limité aux ${_HIST_RENDER_CAP} reçus les plus récents sur ${recus.length} — affinez les filtres (date, commercial, recherche) pour voir le reste.</td></tr>`
    : '';
  tbody.innerHTML = _bandeauTronque + recusAffiches.map(r=>`<tr>
    ...
    <td style="text-align:center;white-space:nowrap;">
      <button class="btn btn-xs" style="background:rgba(120,140,255,0.12);color:#8fa4ff;border:1px solid rgba(120,140,255,0.3)" onclick="reimprimerRecu('${r._id}')" title="Réimprimer ce reçu">🖨️ Réimprimer</button>
      <button class="btn btn-xs" style="background:rgba(201,168,76,0.12);color:var(--accent);border:1px solid rgba(201,168,76,0.3);margin-left:4px;" onclick="envoyerRecuAuCommercial('${r._id}')" title="Envoyer ce reçu au commercial via la messagerie">📨 Envoyer</button>
    </td>
  </tr>`).join('');
}
window.renderHistoriqueRecus = renderHistoriqueRecus;

// Envoi d'un reçu au commercial concerné via la messagerie interne — celui-ci
// reçoit un message avec un bouton lui permettant d'ouvrir et d'imprimer le
// reçu directement (sans avoir accès à l'historique complet des reçus).
window.envoyerRecuAuCommercial = async function(id){
  if(!_recuCheckAccess()) return;
  const r = (DB.recus||[]).find(x=>x._id===id);
  if(!r){ notify('Reçu introuvable','err'); return; }
  const commercialId = r.commercialId;
  if(!commercialId){ notify('Ce reçu n\'est rattaché à aucun commercial','err'); return; }
  const com = DB.commerciaux.find(c=>c._id===commercialId);
  if(!com){ notify('Commercial introuvable','err'); return; }
  const typeLabels = {cotisation:'💳 Cotisation(s)', adhesion:'🤝 Adhésion', rachat_carnet:'📘 Rachat carnet'};
  const texte = `📄 Reçu ${r.recuNum||''} — ${typeLabels[r.type||'cotisation']||'Reçu'} — ${r.clientNom||''} — ${fmt(r.montant||0)}`;
  const convId = _chatPairId(session.userId, commercialId);
  try{
    await _chatEnsureConversation(convId, commercialId);
    const now = Date.now();
    await addDoc(collection(db_fs,'chatMessages'), {
      conversationId: convId,
      type: 'recu',
    ...
    const conv = _chatConversations.get(convId) || {};
    const otherUnread = (conv.unread?.[commercialId] || 0) + 1;
    await setDoc(doc(db_fs,'chatConversations',convId), {
      lastMsg: texte.slice(0,120),
      lastAuthorId: session.userId,
      lastAuthorRole: session.role,
      updatedAt: now,
      unread: {[commercialId]: otherUnread, [session.userId]: 0}
    }, {merge:true});
    notify(`Reçu envoyé à ${com.nom} via la messagerie ✓`);
  } catch(e){
    console.error('[envoyerRecuAuCommercial]', e);
    notify("Erreur lors de l'envoi du reçu","err");
  }
};

// Ouverture d'un reçu depuis un message de chat (staff ou commercial) — va
// chercher le document à jour dans Firestore plutôt que de dépendre d'un
// DB.recus potentiellement non chargé pour le rôle courant (ex : commercial).
window.ouvrirRecuDepuisChat = async function(recuId){
  if(!recuId){ notify('Reçu introuvable','err'); return; }
  try{
    const snap = await getDoc(doc(db_fs,'recus',recuId));
    if(!snap.exists()){ notify('Reçu introuvable ou supprimé','err'); return; }
    const r = snap.data();
    afficherRecu({
      type: r.type,
    ...
    console.error('[ouvrirRecuDepuisChat]', e);
    notify("Accès refusé ou erreur lors de l'ouverture du reçu","err");
  }
};
...
// + Détection de nouvelle version déployée sur GitHub/hébergeur, avec
// bannière visuelle proposant à l'utilisateur de mettre à jour l'app.
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js')
      .then((reg) => {
        // Cas 1 : une version est déjà en attente au moment où l'app se charge
        // (ex: onglet resté ouvert pendant un déploiement).
        if (reg.waiting) _afficherBanniereMAJ(reg.waiting);

        // Cas 2 : une nouvelle version vient d'être détectée (sw.js a changé
        // sur le serveur) et est en cours d'installation.
        reg.addEventListener('updatefound', () => {
          const nouveauWorker = reg.installing;
          ...
          nouveauWorker.addEventListener('statechange', () => {
            ...
            // première installation, c'est une mise à jour d'une version déjà
            // utilisée -> on prévient l'utilisateur.
            if (nouveauWorker.state === 'installed' && navigator.serviceWorker.controller) {
              _afficherBanniereMAJ(nouveauWorker);
            }
          });
        });

        // Revérifie s'il existe une nouvelle version quand l'app revient au
        // premier plan (changement d'onglet, réouverture du téléphone, etc.)
        // — utile car le navigateur ne revérifie pas toujours sw.js assez tôt.
        document.addEventListener('visibilitychange', () => {
          if (document.visibilityState === 'visible') reg.update().catch(()=>{});
        });

        // Revérifie aussi périodiquement (toutes les 20 min) tant que l'onglet
        // est ouvert et visible — utile pour les utilisateurs qui gardent
        // l'app ouverte toute la journée sans jamais changer d'onglet, cas où
        // ils ne recevraient sinon la notification qu'après ~24h.
        const INTERVALLE_VERIF_MAJ = 20 * 60 * 1000; // 20 minutes
        setInterval(() => {
          if (document.visibilityState === 'visible') reg.update().catch(()=>{});
        }, INTERVALLE_VERIF_MAJ);
      })
      .catch(err => console.error('Échec enregistrement Service Worker :', err));

    // Quand le nouveau Service Worker prend le contrôle (juste après que
    // l'utilisateur a cliqué sur "Mettre à jour"), on recharge la page pour
...
// Affiche une bannière discrète en bas de l'écran proposant la mise à jour.
// N'affiche qu'une seule fois même si l'événement se déclenche plusieurs fois.
function _afficherBanniereMAJ(workerEnAttente){
  if (document.getElementById('maj-banniere')) return;

  const bandeau = document.createElement('div');
  bandeau.id = 'maj-banniere';
  bandeau.innerHTML = `
  ...
  document.getElementById('maj-banniere-btn').addEventListener('click', () => {
    workerEnAttente.postMessage({ type: 'SKIP_WAITING' });
    const btn = document.getElementById('maj-banniere-btn');
    if (btn) { btn.disabled = true; btn.textContent = 'Mise à jour…'; }
  });
  document.getElementById('maj-banniere-fermer').addEventListener('click', () => {
    bandeau.remove();
  });
}
