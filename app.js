...
window.sauvegarderEditionClient = async function(){
  ...
  notify(`Client ${nom} mis à jour ✓`,'',cid);
};
...
function setSyncStatus(ok){ document.getElementById('sync-status').innerHTML=`<div class="sync-dot" style="background:${ok?'var(--accent2)':'var(--accent3)'}"></div><span style="color:${ok?'var(--accent2)':'var(--accent3)'}">${ok?'Synchronisé':'Synchronisation...'}</span>`; }
...
window.confirmImportProduits = async function() {
  ...
  btn.disabled = true;
  const total = importProdData.length;
  let count = 0, errCount = 0, lastErr = null;
  ...
  importProdData = [];
  renderProduits();
  let msg = `${count} produit(s) importé(s) avec succès ✓`;
  if(errCount>0) msg += ` — ⚠️ ${errCount} échec(s) (ex: ${lastErr?.message||lastErr})`;
  notify(msg, errCount>0 ? 'err' : 'ok');
};
...
async function renderRegistre(){
  ...
  // Pour l'admin : utilise registreAgenceFilter ('' = toutes, sinon id agence)
  // Pour les autres rôles : utilise sessionAgenceId() comme avant
  const isAdmin = session && ['admin','chef_agence'].includes(session.role);
...
async function renderFiche(){
  ...
  if(isViewer){
    ...
    // Le chef d'agence ne voit que les commerciaux de SA propre agence
    if(isChefAgence){
...
window.saveCl = async function(){
  ...
  try{
    ...
    closeM('m-client');notify(`Client ${nom} créé ✓ — Code : ${codeClient}`,'',newClRef&&newClRef._id?newClRef._id:null);
  }catch(e){
...
// ========= NOTIF =========
// Historisé dans la cloche 🔔 — clientId optionnel : ajoute un lien direct
// vers la fiche client + déclenche son/notification navigateur.
...
function _notifBellPush(msg,t,clientId){
  _notifBellItems.unshift({ id: Date.now()+'_'+Math.random().toString(36).slice(2,7), msg, t, ts: Date.now(), read:false, clientId: clientId||null });
  ...
  // Notification client (créé/modifié…) → son + notification navigateur,
  // même logique que le chat : son toujours, popup navigateur seulement
  // si l'onglet est caché/inactif (pour ne pas doubler avec l'app déjà ouverte).
  if(clientId){
    try{ if(typeof playChatNotifSound==='function') playChatNotifSound(); }catch(e){}
    if(document.hidden || !document.hasFocus()){
      _notifBellNotifNavigateur(msg, clientId);
    }
  }
}
function _notifBellNotifNavigateur(msg, clientId){
  if(!('Notification' in window) || Notification.permission !== 'granted') return;
  try{
    const n = new Notification('🔔 Notification client', {
      body: msg,
      tag: 'notif-bell-'+clientId,
      renotify: true
    });
    n.onclick = ()=>{
      window.focus();
...
function _notifBellRender(){
  ...
  list.innerHTML = _notifBellItems.map(it=>`
    <div class="notif-item${it.t==='err'?' err':''}${it.read?'':' unread'}">
      <span class="notif-item-dot"></span>
      <div class="notif-item-body">
        <div class="notif-item-msg">${esc(it.msg)}</div>
        <div class="notif-item-time">${_notifBellTimeAgo(it.ts)}${it.clientId?' · <a href="#" class="notif-item-link" onclick="notifBellOpenClient(\''+it.clientId+'\');return false;">Voir la fiche →</a>':''}</div>
      </div>
    </div>
  `).join('');
}
window.notifBellOpenClient = function(clientId){
  notifBellToggle(false);
  if(typeof ouvrirFicheClient==='function') ouvrirFicheClient(clientId);
};
...
window.confirmImport = async function(){
  ...
  notifEl.className = 'nitem';
  notifEl.textContent = `⏳ Import en cours… 0 / ${total}`;
  notifEl.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);z-index:9999;background:var(--card);border:1px solid var(--border);padding:10px 20px;border-radius:8px;font-size:13px;';
    ...
    for(let i = 0; i < importData.length; i++){
     try {
      ...
      if(codeClient && clientsExistantsParCode.has(codeClient) && !codesVusDansFichier.has(codeClient)){
        ...
        majClients++;
        count++;
        notifEl.textContent = `⏳ Import en cours… ${count} / ${total}`;
        continue;
      }
      ...
      errCount++;
      lastErr = rowErr;
      notifEl.textContent = `⏳ Import en cours… ${count} / ${total} (${errCount} échec(s))`;
     }
    }
  ...
  importData=[];
  let msg = `${count} ligne(s) traitée(s) avec succès !`;
...
window.confirmImportMvts = async function(){
  ...
  closeM('m-import-mvt');
  importMvtData=[];
  let msg = `✅ ${count} mouvement(s) importé(s) avec succès !`;
  if(errCount>0) msg += ` — ${errCount} échec(s)`;
  notify(msg, errCount>0?'':'ok');
};
...
window.confirmImportArticles = async function(){
  ...
  let msg = `✅ ${count} article(s) importé(s) avec succès !`;
  if(errCount>0) msg += ` — ⚠️ ${errCount} échec(s) (ex: ${lastErr?.message||lastErr})`;
  notify(msg, errCount>0 ? 'err' : 'ok');
};
...
window.saveComNouveauClient = async function(){
  ...
  notify(`Client ${nom} créé — Code : ${codeClient}`,'',newClientRef&&newClientRef._id?newClientRef._id:null);
  // Réinitialiser
