...
let _activeProgressTasks = new Map();
...
try { await deleteApp(current); } catch(e) {  }
...
const _SMALL_COLS = new Set(['commerciaux','agences','articles','produits','primesPaliers','demandesLivraison']);
...
const _AGENCE_SCOPED_COLS_FOR_STAFF = new Set(['mises', 'versements', 'livraisons']);
const _AGENCE_SCOPED_ROLES = new Set([ROLES.SECRETAIRE, ROLES.CHEF_AGENCE]);
function _comIdsForAgence(agenceId) {
return (DB.commerciaux || [])
.filter(c => c.role === ROLES.COMMERCIAL && c.agenceId === agenceId)
.map(c => c._id);
}
function _isAgenceScoped(col, role) {
return _AGENCE_SCOPED_ROLES.has(role) && _AGENCE_SCOPED_COLS_FOR_STAFF.has(col);
}
const _SS_PREFIX = 'gestcom_cache_';
async function _cacheWrite(col, data) {
try {
await idbSet(_SS_PREFIX + col, { t: Date.now(), d: data });
} catch(e) {
console.warn('[cache] échec écriture pour "' + col + '" :', e);
}
}
async function _cacheRead(col) {
try {
const val = await idbGet(_SS_PREFIX + col);
if (!val || typeof val !== 'object' || !('t' in val)) return null;
return val.d;
} catch(e) { return null; }
}
...
const toDelete = keys.filter(k => typeof k === 'string' && k.startsWith(_SS_PREFIX));
await Promise.all(toDelete.map(k => idbDelete(k).catch(()=>{})));
} catch(e) {}
try {
...
(async function _cacheMigrateFromLocalStorage() {
try {
const keys = Object.keys(localStorage).filter(k => k.startsWith(_SS_PREFIX));
...
const cached = await _cacheRead(col);
...
await _cacheWrite(col, DB[col]);
_loadedCols.add(col);
return;
}
const colRef = collection(db_fs, col);
if (_isAgenceScoped(col, role)) {
await _loadColOnce('commerciaux');
const moi = getCom(uid);
const agenceId = moi ? moi.agenceId : null;
const comIds = agenceId ? _comIdsForAgence(agenceId) : [];
if (agenceId && comIds.length && comIds.length <= 30) {
const _agenceCacheKey = col + ':agence:' + agenceId;
const cached = await _cacheRead(_agenceCacheKey);
if (cached) { DB[col] = cached; _loadedCols.add(col); _trackMaxTs(col, cached); return; }
const snap = await getDocs(query(colRef, where('commercialId', 'in', comIds)));
DB[col] = snap.docs.map(d => ({...d.data(), _id: d.id}));
_trackMaxTs(col, DB[col]);
await _cacheWrite(_agenceCacheKey, DB[col]);
_loadedCols.add(col);
return;
}
}
...
const cached = await _cacheRead(_commercialCacheKey);
if (cached) { DB[col] = cached; _loadedCols.add(col); _trackMaxTs(col, cached); return; }
const snap = await getDocs(q);
DB[col] = snap.docs.map(d => ({...d.data(), _id: d.id}));
await _cacheWrite(_commercialCacheKey, DB[col]);
_loadedCols.add(col);
} else {
...
let _regLastSig = null;
let _ficheDateQueryToken = 0;
let _ficheLastSig = null;
const _dateFetchCache = {};
const _DATE_FETCH_TTL_MS = 30 * 1000;
...
const _loadColInFlight = new Map();
...
'demandes-livraison': ['demandesLivraison','produits','agences','commerciaux'],
'catalogue':          ['articles'],
'stock':              ['articles','stockMvts'],
'livraisons':         ['livraisons','clients','commerciaux','paiements','produits','agences'],
'historique':         ['paiements','clients','commerciaux','adhesionPays','agences'],
'registre':           ['paiements','clients','commerciaux','adhesionPays','versements','agences','primesPaliers'],
...
'controle-depart':       ['clients','commerciaux','agences'],
'rapport-activite':      ['paiements','adhesionPays','rachatCarnetPays','livraisons','mises','stockMvts','clients','commerciaux','agences','primesPaliers'],
};
const DELAY_CHART_RENDER_MS  = 80;
const DELAY_CODE_SUGGEST_MS  = 100;
const DELAY_CODE_SUGGEST2_MS = 300;
const DELAY_NOTIFY_HIDE_MS   = 2500;
const DELAY_ANIM_RESET_MS    = 700;
const DELAY_RENDER_DEBOUNCE_MS = 300;
...
const DELAY_USER_ACTIVE_WINDOW_MS = 45000;
['mousemove','keydown','touchstart','scroll'].forEach(ev =>
document.addEventListener(ev, () => { _lastUserActivity = Date.now(); }, {passive:true})
);
function _isUserActive() { return Date.now() - _lastUserActivity < DELAY_USER_ACTIVE_WINDOW_MS; }
...
if (_NO_REALTIME_COLS.has(col)) return null;
if (_NO_REALTIME_FOR_STAFF.has(col) && session.role !== 'commercial') return null;
const colRef = collection(db_fs, col);
...
const comIds = agenceId ? _comIdsForAgence(agenceId) : [];
if (agenceId && comIds.length && comIds.length <= 30) {
return query(colRef, where('commercialId', 'in', comIds));
}
}
if (role ===ROLES.COMMERCIAL) {
...
if (_isAgenceScoped(col, session.role)) {
const moi = getCom(session.userId);
const agenceId = moi ? moi.agenceId : null;
const comIds = agenceId ? _comIdsForAgence(agenceId) : [];
if (agenceId && comIds.length && comIds.length <= 30) return true;
}
return false;
}
...
cols.forEach(col => {
if (_activeListeners.has(col)) return;
const q = _queryForCol(col);
if (!q) return;
const isSoleLoadSource = !_loadedCols.has(col);
let _firstSnap = true;
const _trustRemoved = _listenerCoversFullLoad(col);
const unsub = onSnapshot(q, snap => {
const removedToVerify = [];
...
if (removedToVerify.length) {
removedToVerify.forEach(async id => {
try {
const snap2 = await getDoc(doc(db_fs, col, id));
if (!snap2.exists()) {
DB[col] = (DB[col]||[]).filter(x => x._id !== id);
_touchVer(col);
if (session && curPg && _PAGE_DEPS[curPg]?.includes(col)) renderPg(curPg);
}
} catch(e) {
console.warn('[listener removed-check]', col, id, e);
}
});
}
setSyncStatus(true);
...
commerciauxPrive: ['admin','secretaire'],
...
demandesLivraison: ['admin','chef_agence','gestionnaire_stock'],
...
const _REPLAY_BACKOFF_MS  = [1000, 3000, 8000];
...
if (_activeListeners.has(col) && _listenerCoversFullLoad(col)) continue;
try {
...
let _lastAppliedOnlineState = null;
function _applyOnlineState(online) {
if (_lastAppliedOnlineState === online) return;
if (_lastAppliedOnlineState !== null) _noteNetworkTransition();
...
async function _syncLocalAfterWrite(col) {
try {
const keysToDelete = [_SS_PREFIX + col];
if (session && session.userId) keysToDelete.push(_SS_PREFIX + col + ':' + session.userId);
if (session && session.role !== ROLES.COMMERCIAL) {
const moi = getCom && getCom(session.userId);
if (moi && moi.agenceId) keysToDelete.push(_SS_PREFIX + col + ':agence:' + moi.agenceId);
}
await Promise.all(keysToDelete.map(k => idbDelete(k).catch(()=>{})));
} catch(e) {}
_loadedCols.delete(col);
...
let _sessionInvalidHandled = false;
...
if(data.agenceId !== undefined) return data;
...
_touchVer(col);
await _syncLocalAfterWrite(col);
setSyncStatus(true);
return item;
} catch(e) {
...
} catch(e) { console.warn('saveLocalEncrypted: echec, donnees non persistees localement', e); }
}
let _lid = (() => {
try {
...
const UNSTABLE_WINDOW_MS = 15000;
const UNSTABLE_THRESHOLD = 3;
const UNSTABLE_CLEAR_MS  = 10000;
...
if (_lastSyncStatusOk === ok) return;
...
const _LGA_PALIERS = [60000, 300000, 1800000, 7200000];
...
_sessionInvalidHandled = false;
_resetSessionTimer();
_startTokenRefresh();
_startAlertesLivraison();
} catch(e) {
...
await _initLocalKey(session.userId);
...
const _ALERTES_LIV_INTERVAL_MS = 20 * 60 * 1000;
const _ALERTES_LIV_RENOTIF_MS  = 3 * 60 * 60 * 1000;
...
let _chatConversations = new Map();
let _chatMessages = [];
let _chatConvListReady = false;
...
const TITLES={'admin-dashboard':'Tableau de bord',registre:'Registre par date',historique:'Historique','tous-clients':'Clients',controle:'Contrôle',livraisons:'Livraisons',articles:'Articles',produits:'Produits','demandes-livraison':'Demandes de livraison',catalogue:'Catalogue visuel',stock:'Stock',commerciaux:'Utilisateurs',agences:'Agences',recouvrement:'Recouvrement du jour',fiche:'Fiche du jour','saisie-mises':'Saisie de mises','saisie-adhesions':'Saisie des adhésions','com-clients':'Mes clients','com-nouveau-client':'Nouveau client','rapport-activite':"Rapport d'activité",'gstock-periode':'Suivi de période','transfert-resiliation':'Transfert & Résiliation','depenses-commerciaux':'Dépenses commerciaux','historique-recus':'Historique des reçus','controle-depart':'Contrôle avant départ'};
const BTN={'tous-clients':'+ Client',articles:'+ Article',produits:'+ Produit',livraisons:'+ Livraison',stock:'+ Mouvement',commerciaux:'+ Utilisateur','demandes-livraison':'+ Demande'};
const ACT={'tous-clients':()=>openM('m-client'),articles:()=>openM('m-article'),produits:()=>ouvrirModalProduit(),livraisons:()=>openM('m-livraison'),stock:()=>openM('m-stock-mvt'),commerciaux:()=>openM('m-com'),'demandes-livraison':()=>openModalDemandeLivraison()};
let curPg='';
const BN_CONFIG = {
admin: {
main: [
...
{id:'demandes-livraison', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 12V8H4v4M4 8l1-5h14l1 5M4 12v8h16v-8M4 12h16"/></svg>', label:'Demandes livraison'},
...
{id:'livraisons', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>', label:'Livraisons'},
{id:'produits',  icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>', label:'Produits'},
{id:'demandes-livraison', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 12V8H4v4M4 8l1-5h14l1 5M4 12v8h16v-8M4 12h16"/></svg>', label:'Demandes'},
],
drawer: [
{id:'__bt_printer__', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6.5 6.5 17.5 17.5 12 23 12 1 17.5 6.5 6.5 17.5"/></svg>', label:'Imprimante BT', special:'bt'},
{id:'__logout__', icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>', label:'Déconnexion', special:'logout'},
...
else if(id === 'demandes-livraison') showBtn = ['admin','chef_agence'].includes(role);
...
if(id==='demandes-livraison' && !['admin','chef_agence','gestionnaire_stock','controleur'].includes(session?.role)){ notify('Accès non autorisé','err'); return; }
if((id==='registre' || id==='commerciaux') && session?.role===ROLES.CONTROLEUR){ notify('Accès non autorisé','err'); return; }
try {
if(m[id]) m[id]();
...
let dashPeriod = 30;
let dashAgenceFilter = '';
let registreAgenceFilter = '';
...
if (myDashToken !== _dashQueryToken) return;
...
try { await deleteObject(storageRef(storage, ancienImageUrl)); } catch(e) {  }
}
if (btnSave) { btnSave.disabled = false; btnSave.textContent = '✓ Enregistrer'; }
closeM('m-produit');
renderProduits();
...
catZoomImgIndex = existingImgs.length;
...
let demandeLignesTmp = [];
function _demandeAgenceId(){
const u = DB.commerciaux.find(c=>c._id===session.userId);
return u?.agenceId || '';
}
window.openModalDemandeLivraison = function(){
if(session?.role !== 'chef_agence'){ notify("Accès réservé au chef d'agence",'err'); return; }
demandeLignesTmp = [];
const sel = document.getElementById('dl-produit');
if(sel){
sel.innerHTML = '<option value="">— Sélectionner un produit —</option>' +
DB.produits.slice().sort((a,b)=>(a.nom||'').localeCompare(b.nom||'')).map(p=>`<option value="${p._id}">${esc(p.nom)}</option>`).join('');
}
const qtyEl = document.getElementById('dl-qty'); if(qtyEl) qtyEl.value = 1;
const noteEl = document.getElementById('dl-note'); if(noteEl) noteEl.value = '';
_renderDemandeLignesTmp();
openM('m-demande-livraison');
};
window.dlAjouterLigne = function(){
const sel = document.getElementById('dl-produit');
const pid = sel?.value;
const qty = parseInt(document.getElementById('dl-qty')?.value)||0;
if(!pid){ notify('Choisissez un produit','err'); return; }
if(qty<=0){ notify('Quantité invalide','err'); return; }
const prod = DB.produits.find(p=>p._id===pid);
if(!prod) return;
const existant = demandeLignesTmp.find(l=>l.produitId===pid);
if(existant) existant.qteDemandee += qty;
else demandeLignesTmp.push({produitId:pid, produitNom:prod.nom, qteDemandee:qty});
document.getElementById('dl-qty').value = 1;
if(sel) sel.value = '';
_renderDemandeLignesTmp();
};
window.dlSupprimerLigne = function(pid){
demandeLignesTmp = demandeLignesTmp.filter(l=>l.produitId!==pid);
_renderDemandeLignesTmp();
};
function _renderDemandeLignesTmp(){
const tb = document.getElementById('dl-lignes-body');
if(!tb) return;
if(!demandeLignesTmp.length){
tb.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--muted);padding:14px;">Aucun article ajouté</td></tr>';
return;
}
tb.innerHTML = demandeLignesTmp.map(l=>`<tr>
    <td>${esc(l.produitNom)}</td>
    <td>${l.qteDemandee}</td>
    <td><button class="btn btn-ghost btn-sm" onclick="dlSupprimerLigne('${l.produitId}')">✕</button></td>
  </tr>`).join('');
}
window.saveDemandeLivraison = async function(){
if(!demandeLignesTmp.length){ notify('Ajoutez au moins un article à la demande','err'); return; }
const agenceId = _demandeAgenceId();
if(!agenceId){ notify('Aucune agence associée à votre compte','err'); return; }
await fbAdd('demandesLivraison', {
agenceId,
agenceNom: getAgence(agenceId).nom,
chefAgenceId: session.userId,
chefAgenceNom: session.nom,
statut: 'en_attente',
lignes: demandeLignesTmp.map(l=>({...l, qteEnvoyee:0, statutLigne:'en_attente', delaiJours:0, delaiDate:'', recreee:false})),
dateCreation: TODAY,
note: document.getElementById('dl-note')?.value || ''
});
closeM('m-demande-livraison');
notify('Demande de livraison envoyée ✓');
demandeLignesTmp = [];
renderDemandesLivraison();
};
window.ouvrirTraitementDemande = function(demandeId){
if(!['admin','gestionnaire_stock'].includes(session?.role)){ notify('Accès non autorisé','err'); return; }
const d = DB.demandesLivraison.find(x=>x._id===demandeId);
if(!d){ notify('Demande introuvable','err'); return; }
let overlay = document.getElementById('dl-traiter-overlay');
if(overlay) overlay.remove();
overlay = document.createElement('div');
overlay.id = 'dl-traiter-overlay';
overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9998;display:flex;align-items:center;justify-content:center;padding:20px;';
overlay.onclick = (e)=>{ if(e.target===overlay) overlay.remove(); };
const lignesHtml = d.lignes.map((l,i)=>`
    <div style="border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:10px;background:var(--surface2);">
      <div style="font-weight:700;font-size:13px;margin-bottom:8px;">${esc(l.produitNom)} <span style="color:var(--muted);font-weight:400;">— demandé : ${l.qteDemandee}</span></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <label style="font-size:11px;color:var(--muted);">Qté envoyée</label>
        <input type="number" id="dl-tr-qty-${i}" value="${l.statutLigne==='envoye'?l.qteEnvoyee:l.qteDemandee}" min="0" max="${l.qteDemandee}" style="width:80px;background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:5px 8px;color:var(--text);">
        <label style="font-size:11px;color:var(--muted);margin-left:10px;display:flex;align-items:center;gap:4px;">
          <input type="checkbox" id="dl-tr-indispo-${i}" ${l.statutLigne==='indisponible'?'checked':''} onchange="dlToggleIndispo(${i})"> Indisponible
        </label>
        <span id="dl-tr-delai-wrap-${i}" style="display:${l.statutLigne==='indisponible'?'inline-flex':'none'};align-items:center;gap:6px;">
          <label style="font-size:11px;color:var(--muted);">Délai (jours)</label>
          <input type="number" id="dl-tr-delai-${i}" value="${l.delaiJours||3}" min="1" style="width:60px;background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:5px 8px;color:var(--text);">
        </span>
      </div>
    </div>`).join('');
overlay.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;max-width:560px;width:100%;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;border-bottom:1px solid var(--border);">
        <strong style="font-size:15px;">🚚 Traiter la demande — ${esc(d.agenceNom||'')}</strong>
      ...
      <div style="overflow-y:auto;padding:16px 20px;">
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px;">Demandé par ${esc(d.chefAgenceNom||'')} le ${d.dateCreation}</div>
        ${lignesHtml}
      </div>
      <div style="padding:14px 20px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px;">
        <button class="btn btn-ghost" onclick="document.getElementById('dl-traiter-overlay').remove()">Annuler</button>
        <button class="btn btn-primary" onclick="confirmerTraitementDemande('${d._id}')">✅ Valider</button>
      </div>
    </div>`;
document.body.appendChild(overlay);
};
window.dlToggleIndispo = function(i){
const wrap = document.getElementById('dl-tr-delai-wrap-'+i);
const cb = document.getElementById('dl-tr-indispo-'+i);
if(wrap) wrap.style.display = cb.checked ? 'inline-flex' : 'none';
};
window.confirmerTraitementDemande = async function(demandeId){
const d = DB.demandesLivraison.find(x=>x._id===demandeId);
if(!d) return;
const nouvellesLignes = d.lignes.map((l,i)=>{
const indispo = document.getElementById('dl-tr-indispo-'+i)?.checked;
if(indispo){
const delaiJours = Math.max(1, parseInt(document.getElementById('dl-tr-delai-'+i)?.value)||3);
const delaiDate = new Date(Date.now()+delaiJours*86400000).toISOString().split('T')[0];
return {...l, statutLigne:'indisponible', qteEnvoyee:0, delaiJours, delaiDate, recreee:false};
}
const qty = Math.max(0, Math.min(l.qteDemandee, parseInt(document.getElementById('dl-tr-qty-'+i)?.value)||0));
return {...l, statutLigne:'envoye', qteEnvoyee:qty, delaiJours:0, delaiDate:''};
});
const toutesEnvoyees = nouvellesLignes.every(l=>l.statutLigne==='envoye');
const statut = toutesEnvoyees ? 'cloturee' : 'partielle';
await fbUpdate('demandesLivraison', demandeId, {
lignes: nouvellesLignes,
statut,
dateTraitement: TODAY,
traiteParId: session.userId,
traiteParNom: session.nom
});
document.getElementById('dl-traiter-overlay')?.remove();
notify('Demande traitée ✓');
renderDemandesLivraison();
};
async function _checkRecreationDemandes(){
const today = TODAY;
for(const d of DB.demandesLivraison.slice()){
if(d.statut!=='partielle') continue;
const lignesAReprendre = d.lignes.filter(l=>l.statutLigne==='indisponible' && !l.recreee && l.delaiDate && l.delaiDate<=today);
if(!lignesAReprendre.length) continue;
const majLignes = d.lignes.map(l=>lignesAReprendre.includes(l) ? {...l, recreee:true} : l);
await fbUpdate('demandesLivraison', d._id, {lignes: majLignes});
await fbAdd('demandesLivraison', {
agenceId: d.agenceId,
agenceNom: d.agenceNom,
chefAgenceId: d.chefAgenceId,
chefAgenceNom: d.chefAgenceNom,
statut: 'en_attente',
lignes: lignesAReprendre.map(l=>({produitId:l.produitId, produitNom:l.produitNom, qteDemandee:l.qteDemandee, qteEnvoyee:0, statutLigne:'en_attente', delaiJours:0, delaiDate:'', recreee:false})),
dateCreation: today,
note: 'Générée automatiquement — articles indisponibles de la demande précédente (délai écoulé)',
demandeParentId: d._id
});
}
}
async function renderDemandesLivraison(){
const role = session?.role;
await _checkRecreationDemandes();
let demandes = (DB.demandesLivraison||[]).slice();
if(role==='chef_agence'){
const agenceId = _demandeAgenceId();
demandes = demandes.filter(d=>d.agenceId===agenceId);
}
demandes.sort((a,b)=> (b.dateCreation||'').localeCompare(a.dateCreation||'') || (b._id||'').localeCompare(a._id||''));
const statutBadge = (s)=>({
en_attente: '<span style="font-size:11px;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(201,168,76,0.15);color:var(--warn);">⏳ En attente</span>',
partielle:  '<span style="font-size:11px;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(224,92,82,0.15);color:var(--danger);">⚠️ Partielle</span>',
cloturee:   '<span style="font-size:11px;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(34,212,160,0.15);color:var(--accent2);">✅ Clôturée</span>'
}[s] || s);
const canTraiter = ['admin','gestionnaire_stock'].includes(role);
const cont = document.getElementById('dl-liste');
if(!cont) return;
if(!demandes.length){
cont.innerHTML = '<div style="text-align:center;color:var(--muted);padding:30px;">Aucune demande de livraison</div>';
return;
}
cont.innerHTML = demandes.map(d=>{
const lignesHtml = d.lignes.map(l=>{
const icon = l.statutLigne==='envoye' ? '✅' : l.statutLigne==='indisponible' ? '⛔' : '⏳';
const detail = l.statutLigne==='envoye' ? `envoyé ${l.qteEnvoyee}/${l.qteDemandee}`
: l.statutLigne==='indisponible' ? `indisponible — délai jusqu'au ${l.delaiDate}${l.recreee?' (nouvelle demande générée)':''}`
: `demandé ${l.qteDemandee}`;
return `<div style="font-size:12px;padding:3px 0;">${icon} ${esc(l.produitNom)} — ${detail}</div>`;
}).join('');
return `<div style="border:1px solid var(--border);border-radius:12px;padding:14px 16px;margin-bottom:12px;background:var(--surface);">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
        <div>
          <strong style="font-size:13px;">🏢 ${esc(d.agenceNom||'')}</strong>
          <span style="font-size:11px;color:var(--muted);margin-left:8px;">par ${esc(d.chefAgenceNom||'')} · ${d.dateCreation}</span>
        </div>
        ${statutBadge(d.statut)}
      </div>
      ${lignesHtml}
      ${d.note ? `<div style="font-size:11px;color:var(--muted);margin-top:6px;font-style:italic;">${esc(d.note)}</div>` : ''}
      ${canTraiter && d.statut!=='cloturee' ? `<div style="margin-top:10px;"><button class="btn btn-primary btn-sm" onclick="ouvrirTraitementDemande('${d._id}')">Traiter</button></div>` : ''}
    </div>`;
}).join('');
}
...
if(btnLivInline) btnLivInline.style.display = ['admin','secretaire','chef_agence'].includes(session?.role) ? '' : 'none';
...
if (myToken !== _clientsLazyLoadToken) return;
const sel=document.getElementById('filter-com');
const comsVisibles = comsDansAgence().filter(c=>c.role===ROLES.COMMERCIAL);
...
const selQrt = document.getElementById('rc-quartier');
const quartiers = [...new Set(clientsDansAgence().map(c=>(c.quartier||'').trim()).filter(Boolean))]
.sort((a,b)=>a.localeCompare(b,'fr',{sensitivity:'base'}));
selQrt.innerHTML = `<option value="">— Tous les quartiers —</option>`+quartiers.map(q=>`<option value="${esc(q)}">${esc(q)}</option>`).join('');
document.getElementById('rc-statut').value = 'tous';
document.getElementById('rc-tri').value = 'az';
document.getElementById('rc-periode').value = 'toutes';
document.getElementById('rc-date-debut').value = '';
document.getElementById('rc-date-fin').value = '';
_rcTogglePeriodePerso();
_rcTogglePeriodeZone();
openM('m-rapport-controle');
};
window._rcTogglePeriodeZone = function(){
const zone = document.getElementById('rc-periode-zone');
const estNonControleSeul = document.getElementById('rc-statut').value === 'non_controle';
if(zone) zone.style.display = estNonControleSeul ? 'none' : '';
};
window._rcTogglePeriodePerso = function(){
const perso = document.getElementById('rc-periode').value === 'perso';
const zone = document.getElementById('rc-periode-perso-zone');
if(zone) zone.style.display = perso ? 'flex' : 'none';
};
function _rcBornesPeriode(){
const p = document.getElementById('rc-periode').value;
if(p==='toutes') return null;
const today = new Date();
const iso = d => d.toISOString().slice(0,10);
if(p==='semaine'){
const jour = today.getDay();
const decalage = jour===0 ? 6 : jour-1;
const lundi = new Date(today); lundi.setDate(today.getDate()-decalage);
const dimanche = new Date(lundi); dimanche.setDate(lundi.getDate()+6);
return [iso(lundi), iso(dimanche)];
}
if(p==='mois'){
...
if(p==='perso'){
const d = document.getElementById('rc-date-debut').value;
const f = document.getElementById('rc-date-fin').value;
if(!d && !f) return null;
return [d||'0000-01-01', f||'9999-12-31'];
}
return null;
}
...
const quartierFiltre = document.getElementById('rc-quartier').value;
const triOrdre = document.getElementById('rc-tri').value;
const bornesPeriode = _rcBornesPeriode();
let cls = clientsDansAgence();
if(comFiltre) cls = cls.filter(c=>c.commercialId===comFiltre);
if(quartierFiltre) cls = cls.filter(c=>(c.quartier||'').trim()===quartierFiltre);
const enAttenteTotal = cls.filter(c=>!c.controle || c.controle.statut!=='controle');
const dejaFaitTotal  = cls.filter(c=>c.controle && c.controle.statut==='controle');
const dejaFaitPeriode = bornesPeriode
? dejaFaitTotal.filter(c => c.controle.date && c.controle.date>=bornesPeriode[0] && c.controle.date<=bornesPeriode[1])
: dejaFaitTotal;
let liste;
if(statutFiltre==='controle') liste = dejaFaitPeriode;
else if(statutFiltre==='non_controle') liste = enAttenteTotal;
else liste = [...dejaFaitPeriode, ...enAttenteTotal];
liste = [...liste].sort((a,b)=>{
const cmp = (a.nom||'').localeCompare(b.nom||'','fr',{sensitivity:'base'});
return triOrdre==='za' ? -cmp : cmp;
});
const total = dejaFaitPeriode.length + enAttenteTotal.length;
const nbFait = dejaFaitPeriode.length;
const nbAttente = enAttenteTotal.length;
const taux = total>0 ? Math.round((nbFait/total)*100) : 0;
const idsControlesPeriode = new Set(dejaFaitPeriode.map(c=>c._id));
const parCom = {};
[...dejaFaitPeriode, ...enAttenteTotal].forEach(c=>{
const nom = getCom(c.commercialId).nom || '—';
if(!parCom[nom]) parCom[nom] = {total:0, fait:0};
parCom[nom].total++;
if(idsControlesPeriode.has(c._id)) parCom[nom].fait++;
});
...
const quartierLabel = quartierFiltre || 'Tous les quartiers';
const periodeLabelMap = {toutes:'Toutes les dates', semaine:'Cette semaine', mois:'Ce mois-ci', perso:'Période personnalisée'};
const periodeSel = document.getElementById('rc-periode').value;
const periodeLabel = periodeSel==='perso' && bornesPeriode
? `${bornesPeriode[0]} → ${bornesPeriode[1]}`
: periodeLabelMap[periodeSel];
...
const quartierCell = esc(c.quartier||'—');
const sNiv = stats(c);
const dureeC = c.duree||0;
const niveauOk = dureeC>0 && sNiv.joursCouv>=dureeC;
const niveauCell = dureeC>0
? `<span style="font-weight:700;color:${niveauOk?'#1a8a4a':'#333'};">${joursEnJM(sNiv.joursCouv)}</span><span style="color:#999;"> / ${joursEnJM(dureeC)}</span>`
: `<span style="color:#aaa;">—</span>`;
return `<tr><td style="padding:6px 8px;">${esc(c.nom)}${c.codeClient?` (${esc(c.codeClient)})`:''}</td><td style="padding:6px 8px;">${esc(c.tel||'—')}</td><td style="padding:6px 8px;">${esc(getCom(c.commercialId).nom)}</td><td style="padding:6px 8px;text-align:right;">${cotisCell}</td><td style="padding:6px 8px;">${statutCell}</td><td style="padding:6px 8px;">${quartierCell}</td><td style="padding:6px 8px;white-space:nowrap;">${niveauCell}</td><td style="padding:6px 8px;max-width:220px;">${noteCell}</td></tr>`;
}).join('') || `<tr><td colspan="8" style="padding:10px;text-align:center;color:#888;">Aucun client</td></tr>`;
const now = new Date();
...
<body>
  ...
  <div class="sub">Généré le ${dateGen} · Filtre : ${esc(statutLabel)} · Commercial : ${esc(comLabel)} · Quartier : ${esc(quartierLabel)} · Période des contrôles : ${esc(periodeLabel)} · Tri : ${triOrdre==='za'?'Z → A':'A → Z'}</div>
  <div class="kpis">
    <div class="kpi"><div class="lbl">Total clients</div><div class="val">${total}</div></div>
    <div class="kpi"><div class="lbl">✅ Contrôlés${bornesPeriode?' (période)':''}</div><div class="val" style="color:#1a8a4a;">${nbFait}</div></div>
  ...
  <table><thead><tr><th>Client</th><th>Téléphone</th><th>Commercial</th><th style="text-align:right;">Cotisation</th><th>Statut</th><th>Quartier</th><th>Niveau actuel</th><th>Note du contrôleur</th></tr></thead><tbody>${lignesDetail}</tbody></table>
...
if (myHistToken !== _histDateQueryToken) return;
}
...
if (myToken !== _dateQueryToken) return;
...
if (myFicheToken !== _ficheDateQueryToken) return;
const com=getCom(comId);
const cls=clsFiche;
...
window.openPay = function(cid){
  ...
  if(btnConfirm){
    // ✅ Repart toujours d'un texte propre (au cas où resté sur "⏳
    // Enregistrement…" suite à une fermeture pendant un envoi en cours)
...
window.savePay = async function(){
  ...
  // ✅ Avertissement doublon : ce formulaire (encaissement rapide depuis la
  // fiche client) n'avait aucune confirmation avant envoi. On n'en ajoute
  // une que si un paiement existe déjà aujourd'hui pour ce client — pour ne
  // pas ralentir la saisie normale, mais forcer une confirmation explicite
  // dans le cas à risque (voir _avertissementDoublonPaiement plus haut).
  const avertDoublonPay = _avertissementDoublonPaiement(payCtx, payDate);
  if(avertDoublonPay){
    if(!(await confirmDialog(`Client : ${esc(c.nom)}\nMontant : ${fmt(amt)}${avertDoublonPay}`,
      {title:'🚨 Paiement déjà saisi aujourd\'hui', okLabel:'Oui, c\'est un nouveau paiement', danger:true}))) return;
  }

  try{
    // ✅ FIX DOUBLONS : désactive le bouton pendant l'envoi. Sans ça, sur une
    // connexion lente/instable (terrain, 2G), un clic sans retour visuel
    // immédiat pousse à retaper "Confirmer" avant que la 1ère requête ait
    // fini — deux paiements identiques (même montant, même minute) partent
    // alors vers Firestore comme deux écritures indépendantes et légitimes
    // (la protection anti-doublon _offlineId ne s'applique qu'au mode
    // hors-ligne, pas à deux clics en ligne). Résultat visible en fiche du
    // jour : badge "×2 mises" alors qu'il ne s'agit que d'un seul paiement
    // voulu par le commercial.
    const btnPay = document.getElementById('pay-btn-confirm');
    if(btnPay){ btnPay.disabled = true; btnPay.textContent = '⏳ Enregistrement…'; }
    ...
    const btnPay = document.getElementById('pay-btn-confirm');
    if(btnPay){ btnPay.disabled = false; btnPay.textContent = '✓ Confirmer'; }
    return;
  }
...
function chargerArticlesSelect(containerId, prefix){  }
function majMontantArticles(prefix){  }
...
if(!nom||!tel||!ct||mt<=0||!debut){notify('Champs obligatoires manquants','err');return;}
if(window._saveClEnCours){ return; }
window._saveClEnCours = true;
const btnCl = document.getElementById('cl-btn-confirm');
if(btnCl){ btnCl.disabled = true; btnCl.style.opacity = '0.5'; btnCl.style.cursor = 'not-allowed'; btnCl.textContent = '⏳ Enregistrement…'; }
...
window._saveClEnCours = false;
if(btnCl){ btnCl.disabled = false; btnCl.style.opacity = ''; btnCl.style.cursor = ''; btnCl.textContent = 'Enregistrer'; }
}
};
...
if(!['admin','secretaire','commercial','chef_agence'].includes(session?.role)){ notify('Accès refusé','err'); return; }
...
const btnClReset = document.getElementById('cl-btn-confirm');
if(btnClReset){ btnClReset.disabled = false; btnClReset.style.opacity = ''; btnClReset.style.cursor = ''; btnClReset.textContent = 'Enregistrer'; }
}
if(id==='m-com'){
...
_cfmToken = Math.random().toString(36).slice(2);
_cfmResolve = resolve;
...
const NOTIF_BELL_ALLOWED_ROLES = [ROLES.SECRETAIRE, ROLES.CHEF_AGENCE, ROLES.ADMIN, ROLES.COMMERCIAL, ROLES.CONTROLEUR];
...
const doublon = estDoublonFichier;
...
await _syncLocalAfterWrite('clients');
await _syncLocalAfterWrite('paiements');
} finally {
task.finish();
}
...
let importArticlesData = [];
let importArticlesRaw  = [];
...
const btnComNclReset = document.getElementById('com-ncl-btn-confirm');
if(btnComNclReset){ btnComNclReset.disabled = false; btnComNclReset.style.opacity = ''; btnComNclReset.style.cursor = ''; btnComNclReset.textContent = '✓ Créer le client'; }
...
if(window._saveComNclEnCours){ return; }
window._saveComNclEnCours = true;
const btnComNcl = document.getElementById('com-ncl-btn-confirm');
if(btnComNcl){ btnComNcl.disabled = true; btnComNcl.style.opacity = '0.5'; btnComNcl.style.cursor = 'not-allowed'; btnComNcl.textContent = '⏳ Création…'; }
...
console.error('Échec création client:', e);
notify("Échec de la création du client — vérifiez votre connexion et réessayez. Si le problème persiste, contactez l'administrateur.", 'err');
window._saveComNclEnCours = false;
if(btnComNcl){ btnComNcl.disabled = false; btnComNcl.style.opacity = ''; btnComNcl.style.cursor = ''; btnComNcl.textContent = '✓ Créer le client'; }
return;
}
...
miseClientCtx = null;
const btnMiseReset = document.getElementById('mise-saisie-btn-confirm');
if(btnMiseReset){ btnMiseReset.disabled = false; btnMiseReset.style.opacity = ''; btnMiseReset.style.cursor = ''; btnMiseReset.textContent = '✅ Confirmer le paiement — irrévocable'; }
...
function _paiementsDejaAujourdhui(clientId, date){
return (DB.paiements||[]).filter(p =>
p.clientId === clientId && p.date === date
&& p.origine !== 'import_historique' && p.source !== 'transfert'
);
}
function _avertissementDoublonPaiement(clientId, date){
const existants = _paiementsDejaAujourdhui(clientId, date);
if(!existants.length) return '';
const dernier = existants[existants.length-1];
const heures = existants.map(p=>p.heure||'—').join(', ');
return `\n\n⚠️ ATTENTION : ${existants.length} paiement(s) déjà enregistré(s) aujourd'hui pour ce client (à ${heures}, dernier : ${fmt(dernier.montant||0)}).\nÊtes-vous sûr(e) qu'il s'agit bien d'un NOUVEAU paiement, et non d'une resaisie du précédent ?`;
}
...
if(window._miseSaisieEnCours){ return; }
window._miseSaisieEnCours = true;
try{
...
const avertDoublon = _avertissementDoublonPaiement(miseClientCtx, dateChoisie);
const okLabelConfirm = avertDoublon ? 'Oui, c\'est un nouveau paiement' : 'Confirmer le paiement';
if(!(await confirmDialog(`Client : ${esc(c.nom)}\nCotisations : ${nb}x\nMontant : ${fmt(montant)}${dateMention}${avertDoublon}\n\nCe paiement ne pourra pas être modifié.`,{title:avertDoublon?'🚨 Paiement déjà saisi aujourd\'hui':'⚠️ Confirmation irréversible',okLabel:okLabelConfirm,danger:true}))) return;
const btnMise = document.getElementById('mise-saisie-btn-confirm');
if(btnMise){ btnMise.disabled = true; btnMise.style.opacity = '0.5'; btnMise.style.cursor = 'not-allowed'; btnMise.textContent = '⏳ Enregistrement…'; }
try{
...
if(btnMise){ btnMise.disabled = false; btnMise.style.opacity = ''; btnMise.style.cursor = ''; btnMise.textContent = '✅ Confirmer le paiement — irrévocable'; }
return;
}
await new Promise(r=>setTimeout(r,350));
...
window._miseSaisieEnCours = false;
}
};
...
const canSaisir = ['admin','commercial','chef_agence'].includes(session?.role);
const canRachat = ['admin','secretaire','chef_agence'].includes(session?.role);
...
window.setRapportPeriod = function(p){
  ...
  // ✅ FIX CRASH SILENCIEUX : ce point d'entrée (clic sur un onglet de
  // période) appelait renderRapportActivite() sans aucune protection —
  // contrairement au premier affichage de la page (protégé par le
  // try/catch de renderPg). Une erreur ici restait donc totalement
  // silencieuse pour l'utilisateur, avec la page bloquée à mi-rendu.
  try { renderRapportActivite(); }
  catch(e) {
    console.error('[setRapportPeriod] Erreur lors du calcul du rapport :', e);
    notify("⚠️ Erreur lors du calcul du rapport pour cette période.", 'err');
  }
};
...
function renderRapportActivite(){
  ...
  // ✅ FIX PERFORMANCE (cause réelle du plantage/gel du navigateur) : cette
  // section faisait, pour CHAQUE paiement et CHAQUE rachat, un .find() sur
  // la liste ENTIÈRE des clients (recherche linéaire) — et pire, pour CHAQUE
  // commercial, un nouveau passage complet sur TOUS les paiements de la
  // période avec un .find() client à chaque fois (primesAtteintes ET
  // perfCom, chacun indépendamment). Avec une vraie base (plusieurs
  // milliers de clients, dizaines de commerciaux, milliers de paiements sur
  // un trimestre/semestre/année), ça représentait des dizaines à centaines
  // de millions d'opérations — assez pour geler complètement l'onglet
  // plusieurs secondes, voire le faire tuer par le navigateur sur un
  // appareil modeste ("l'app plante"). Remplacé par un index client_id→client
  // (Map, lecture en O(1)) construit UNE seule fois, et un regroupement des
  // paiements/rachats par commercial fait en un seul passage — la
  // complexité globale passe de O(commerciaux × paiements × clients) à
  // O(paiements + rachats + clients + commerciaux), donc linéaire.
  const clientById = new Map((DB.clients||[]).map(c=>[c._id,c]));
  ...
  // Regroupement paiements/rachats par commercial — un seul passage sur
  // chaque liste (au lieu d'un passage complet PAR commercial ci-dessus).
  const paiementsParCom = new Map(); // commercialId -> {ca, nb}
  const touch = (map,id) => { if(!map.has(id)) map.set(id,{ca:0,nb:0}); return map.get(id); };
  paiements.forEach(p=>{
    const cl = clientById.get(p.clientId);
    if(cl && cl.commercialId){ const e=touch(paiementsParCom, cl.commercialId); e.ca+=(Number(p.montant)||0); e.nb++; }
  });
  const rachatsParCom = new Map();
  rachats.forEach(r=>{
    if(r.commercialId){ const e=touch(rachatsParCom, r.commercialId); e.ca+=(Number(r.montant)||0); }
  });
  const clientsParCom = new Map();
  clients.forEach(c=>{
    if(c.commercialId){ const e=touch(clientsParCom, c.commercialId); e.nb++; }
  });

  // Primes atteintes — chercher commerciaux ayant atteint un palier sur la période
  const paliersTries = [...primes].sort((a,b)=>b.seuil-a.seuil); // trié une seule fois, réutilisé pour tous les commerciaux
  const primesAtteintes = [];
  coms.forEach(com=>{
    const totalCom = (paiementsParCom.get(com._id)?.ca||0) + (rachatsParCom.get(com._id)?.ca||0);
    const palier = paliersTries.find(pr=>totalCom>=(Number(pr.seuil)||0));
    // ✅ FIX CHAMPS INEXISTANTS : les documents primesPaliers ont les champs
    // {seuil, montant, label} (voir ajouterPalier) — pas {nom, prime}. Avec
    // les mauvais noms, palier.nom était toujours undefined (repli correct
    // sur "Palier X") mais palier.prime était TOUJOURS undefined→0 : chaque
    // ligne de cette section affichait donc une prime à 0 FCFA, quel que
    // soit le palier réellement configuré.
    if(palier) primesAtteintes.push({com:com.nom, palier:palier.label||`Palier ${palier.seuil}`, montant:palier.montant||0, ca:totalCom});
  });

  // Performance par commercial
  const perfCom = coms.map(com=>{
    const pay = paiementsParCom.get(com._id) || {ca:0, nb:0};
    const rach = rachatsParCom.get(com._id) || {ca:0};
    const cl = clientsParCom.get(com._id) || {nb:0};
    return { nom:com.nom, ca: pay.ca + rach.ca, nbClients: cl.nb, nbPay: pay.nb };
  }).sort((a,b)=>b.ca-a.ca);
...
`;
  ...
  // ✅ FIX CRASH SILENCIEUX : ce bloc s'exécute dans requestAnimationFrame,
  // donc APRÈS que renderPg()/setupPageListeners() ait déjà quitté son
  // try/catch — une erreur ici (ex: Chart.js jette si des données ne sont
  // pas au format attendu) n'était donc JAMAIS rattrapée : elle remontait
  // comme exception non gérée dans la console, sans aucun message pour
  // l'utilisateur, laissant la page à moitié affichée (KPI visibles, mais
  // aucun graphique, et rien d'autre ne réagissait plus sur cette page —
  // ce qui donne l'impression que "l'app plante").
  requestAnimationFrame(()=>{
   try{
    ...
    if(c5){
     ...
     console.error('[renderRapportActivite] Erreur lors de la génération des graphiques :', e);
     notify("⚠️ Les graphiques du rapport n'ont pas pu s'afficher, mais les chiffres ci-dessus restent à jour.", 'err');
   }
  });
}
...
if(q.length < 2){ box.style.display='none'; return; }
const results = (DB.clients||[]).filter(c=>{
if(c._id===_trSrcId) return false;
return (c.nom||'').toLowerCase().includes(q)
|| (c.codeClient||'').toLowerCase().includes(q)
|| (c.tel||'').includes(q);
}).slice(0,8);
...
const INTERVALLE_VERIF_MAJ = 20 * 60 * 1000;
setInterval(() => {
if (document.visibilityState === 'visible') reg.update().catch(()=>{});
